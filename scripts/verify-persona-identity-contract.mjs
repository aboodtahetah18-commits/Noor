...
const assetFile=path.join(root,'src/components/conversations/persona-assets.ts');
const workspaceFile=path.join(root,'src/components/conversations/persistent-conversation-workspace.tsx');
const roleSheetFile=path.join(root,'src/components/conversations/algorithm-role-mobile-sheet.tsx');

const assetSource=fs.readFileSync(assetFile,'utf8');
const workspace=fs.readFileSync(workspaceFile,'utf8');
const roleSheet=fs.readFileSync(roleSheetFile,'utf8');

const mappings=new Map(
  [...assetSource.matchAll(/'([^']+)':'([^']+)'/g)].map(match=>[match[1],match[2]])
);
...
for(const key of requiredPersonaKeys){
  ...
  if(!asset){
    failures.push(`missing persona mapping: ${key}`);
    continue;
  }
...
if(workspace.includes('namaa-algorithmic-personas.jpg')&&workspace.includes('entityPersonaReference')){
  failures.push('aggregate persona sheet must not be rendered in the team view');
}
...
if(failures.length){
  console.error('PERSONA-IDENTITY-CONTRACT-FAIL');
