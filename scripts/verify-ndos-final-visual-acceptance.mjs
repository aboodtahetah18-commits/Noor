...
if (!(componentIndex < ndosIndex && pagesIndex < ndosIndex && ndosIndex < enforcementIndex)) fail(layoutFile, 'import authority must be components/pages -> NDOS -> enforcement');
...
const roots = ['src/app','src/components','src/features','src/design-system'];
...
for (const file of files) {
  ...
  if (/Tajawal/i.test(text)) fail(file, 'legacy Tajawal reference');
  if (/(?:linear|radial|conic)-gradient\s*\(/i.test(text)) fail(file, 'unapproved gradient');
  if (/backdrop-filter\s*:/i.test(text)) fail(file, 'unapproved blur/glass effect');
  ...
  if (file.endsWith('.css')) {
    // RTL-first runtime styling must use logical properties. `left/right` inside transforms or prose is not matched.
    ...
    for (const match of text.matchAll(physical)) fail(file, `physical RTL property ${match[2]}; use logical inline/block property`);
...
console.log(`NDOS-FINAL-VISUAL-ACCEPTANCE-PASS ${files.length} visual source files audited`);
