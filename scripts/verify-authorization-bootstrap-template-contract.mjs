...
const authorizationRepository=read('src/repositories/authorization-repository.ts');
...
requireText(bootstrapMigration,'principal_user_id','ADMINISTER grants must support principal scoping');
requireText(bootstrapMigration,'ADMINISTER_GRANT_REQUIRES_PRINCIPAL','database must reject role-wide ADMINISTER grants');
...
requireText(policy,'ADMINISTER_GRANT_REQUIRES_PRINCIPAL','application policy must reject unscoped ADMINISTER grants');
requireText(authorizationRepository,'principal_user_id is null or principal_user_id=','authorization engine must honor principal-scoped grants');
...
requireText(navRepository,'g.principal_user_id=a.user_id','navigation visibility must honor ADMINISTER principal scope');
