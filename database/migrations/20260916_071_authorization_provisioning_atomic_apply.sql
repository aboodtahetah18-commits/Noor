...
begin
  ...
  if v_kind = 'ROLE_ASSIGNMENT_CREATE' then
    ...
    values (
      ...
      p->>'role',
      nullif(p->>'bankKey',''),
      nullif(p->>'committeeId',''),
      nullif(p->>'serviceId',''),
      'ACTIVE',
      coalesce(nullif(p->>'startsAt','')::timestamptz, now()),
      nullif(p->>'endsAt','')::timestamptz,
      p_actor_user_id,
      p_request_id
    );
    ...
    if not (
      v_current_status = v_status or
      (v_current_status='ACTIVE' and v_status in ('SUSPENDED','REVOKED','EXPIRED')) or
      (v_current_status='SUSPENDED' and v_status in ('ACTIVE','REVOKED','EXPIRED'))
    ) then raise exception 'ROLE_ASSIGNMENT_STATUS_TRANSITION_INVALID'; end if;
    ...
    values (
      ...
      nullif(p->>'caseType',''),
      nullif(p->>'maxRisk',''),
      nullif(p->>'maxMateriality',''),
      nullif(p->>'maxAmount','')::numeric,
      coalesce(nullif(p->>'policyVersion',''),'RBAC_ABAC_v1.0'),
      true,
      p_request_id
    );
    ...
    update public.authorization_grants
      set is_active = coalesce((p->>'isActive')::boolean,false)
    ...
    values (
      ...
      coalesce(p->'scope','{}'::jsonb),
      nullif(p->>'maxAmount','')::numeric,
      nullif(p->>'maxRisk',''),
      (p->>'startsAt')::timestamptz,
      (p->>'endsAt')::timestamptz,
      coalesce((p->>'canRedelegate')::boolean,false),
      r.approved_by,
      'ACTIVE',
      p_request_id
    );
    ...
    if not (
      ...
      (v_current_status='ACTIVE' and v_status in ('SUSPENDED','REVOKED','EXPIRED')) or
      (v_current_status='SUSPENDED' and v_status in ('ACTIVE','REVOKED','EXPIRED'))
    ) then raise exception 'DELEGATION_STATUS_TRANSITION_INVALID'; end if;
    ...
    if not exists (
      ...
      where user_id=(p->>'actorUserId')::uuid
        and role=v_role
        and status='ACTIVE'
        and starts_at<=now()
    ...
    values (
      ...
      r.approved_by,
      coalesce(p->'permissions','[]'::jsonb),
      coalesce(p->'scope','{}'::jsonb) || jsonb_build_object('role',v_role),
      nullif(p->>'maxAmount','')::numeric,
      nullif(p->>'maxRisk',''),
      p->>'justification',
      p->>'incidentReference',
  ...
  values (
    gen_random_uuid(), p_actor_user_id, 'PROVISIONING_APPLIED', p_request_id, r.subject_user_id,
    v_object_type, v_object_id::text, 'Approved authorization provisioning request applied atomically.',
    jsonb_build_object('changeType', r.change_type)
  );
