...
export async function setInitialPassword(input: { token: string; password: string }) {
  ...
  await rawSql.transaction([
    ...
    `,
    rawSql`
      insert into auth.account (id, account_id, provider_id, user_id, password, issuer, created_at, updated_at)
      values (${randomUUID()}::uuid, ${userId}, 'credential', ${userId}::uuid, ${passwordHash}, 'local:credential', now(), now())
    `,
