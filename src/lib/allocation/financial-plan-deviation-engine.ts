...
export async function createFinancialPlanDeviationReplies(userId:string){
  ...
  views.push({
    ...
    body:transferOptions.length
      ? `فتحت حالة الانحراف ${deviation.caseId}. الخيارات الحالية مرقمة بالترتيب: 1) الإبقاء على الخطة، 2) فتح إعادة تفاوض، ثم مقترحات النقل ابتداءً من الخيار 3. لاعتماد خيار اكتب «اعتماد الخيار رقم». أي نقل يظل مشروع تعديل ولا يصبح نافذًا قبل اعتمادك وإصدار نسخة جديدة من الخطة.`
