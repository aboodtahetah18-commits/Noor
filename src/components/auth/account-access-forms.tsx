...
export function RegisterForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/auth/register', { method: 'POST', credentials: 'same-origin', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name, email }) });
  ...
  if (sent) return <div className="auth-success" role="status"><strong>تحقق من بريدك الإلكتروني</strong><p>إذا كان البريد متاحًا للتسجيل فستصلك رسالة من نماء. افتح الرابط لتأكيد البريد ثم إنشاء كلمة المرور.</p><Link href="/login">العودة إلى تسجيل الدخول</Link></div>;
  return <form className="auth-form auth-form-v42 auth-form-nature" onSubmit={submit}>
    <label className="auth-input-field"><span>الاسم</span><span className="auth-input-shell"><input name="name" autoComplete="name" required maxLength={120} disabled={pending} /></span></label>
    <label className="auth-input-field"><span>البريد الإلكتروني</span><span className="auth-input-shell"><input name="email" type="email" autoComplete="email" inputMode="email" required disabled={pending} placeholder="name@example.com" /></span></label>
    {error ? <p className="form-error auth-alert" role="alert">{error}</p> : null}
    <button className="auth-submit" type="submit" disabled={pending}>{pending ? 'جاري الإرسال...' : 'إرسال رابط التحقق'}</button>
...
export function ForgotPasswordForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(true); setError(null);
    try {
      const response = await fetch('/api/auth/forgot-password', { method: 'POST', credentials: 'same-origin', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email }) });
      const body = await response.json().catch(() => ({})) as { code?: string };
  ...
  return <form className="auth-form auth-form-v42 auth-form-nature" onSubmit={submit}>
    <label className="auth-input-field"><span>البريد الإلكتروني</span><span className="auth-input-shell"><input name="email" type="email" autoComplete="email" required disabled={pending} /></span></label>
    {error ? <p className="form-error auth-alert" role="alert">{error}</p> : null}
...
export function PasswordTokenForm({ token, mode }: { token: string; mode: 'setup' | 'reset' }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(true); setError(null);
    try {
      const endpoint = mode === 'setup' ? '/api/auth/set-password' : '/api/auth/reset-password';
  ...
  if (done) return <div className="auth-success" role="status"><strong>{mode === 'setup' ? 'تم إنشاء كلمة المرور' : 'تم تحديث كلمة المرور'}</strong><p>يمكنك الآن تسجيل الدخول إلى نماء.</p><Link href="/login">الانتقال إلى تسجيل الدخول</Link></div>;
  return <form className="auth-form auth-form-v42 auth-form-nature" onSubmit={submit}>
