...
import { GovernanceCenter } from '@/components/conversations/governance-center';
import { openBankMessageDialog } from '@/components/bank-message-dialog';
...
type OnboardingStatus = { status:string; current_step:string; complete:boolean; completed_at?:string|null; question?:string|null };
...
const rooms: [Room, ...Room[]] = [
  ...
  { id:'operations', title:'العمليات والمطابقة', subtitle:'رسائل المشتريات والحركات', lead:'مركز العمليات والمطابقة', specialists:'محرك المطابقة والتصنيف والتسوية', avatar:'/brand/bank-central.webp', bankLogo:'/brand/bank-central.webp' },
  ...
  { id:'secretary', title:'أمين السر المركزي', subtitle:'الحوكمة والاجتماعات والمحاضر', lead:'أمين السر المركزي', specialists:'تنظيم المجلس واللجان واسترجاع السجلات الحاكمة', avatar:'/brand/namaa-logo.webp', bankLogo:'/brand/bank-central.webp' },
  ...
  useEffect(()=>{ let cancelled=false; fetch(`/api/conversations/${activeRoomId}`,{cache:'no-store'})
    ...
    .then(data=>{ if(!cancelled){
      ...
      if(onboarding){
        ...
        setOnboardingCompletedAt(onboarding.completed_at??null);
  ...
  return <section className={styles.page} dir="rtl" aria-label="محادثات نماء">
    ...
    <div className={`${styles.workspace} ${desktopRoomsVisible?'':styles.withoutRooms} ${desktopContextVisible?'':styles.withoutContext}`}>
      ...
      <main className={`${styles.chatPane} ${mobileRoomList?styles.mobileChatHidden:''}`}><header className={styles.chatHeader}><div className={styles.chatIdentity}>{onboardingComplete!==false&&<button type="button" className={styles.mobileBack} aria-label="العودة إلى المحادثات" onClick={()=>setMobileRoomList(true)}><LucideIcon name="chevronRight" size={20}/></button>}<RoomPortrait room={activeRoom} size="md"/><div><div className={styles.entityTitle}><strong>{chatRoleTitle(activeRoom)}</strong></div><small>{chatEntityTitle(activeRoom)}</small></div></div><div className={styles.mobileTools}><button type="button" aria-label="معلومات الجهة" onClick={()=>setContextOpen(true)}><LucideIcon name="info" size={20}/></button></div></header>
        <div className={styles.routingNote}><LucideIcon name="sparkles" size={16}/><span>{activeRoom.specialists}</span></div>{activeRoomId==='operations'&&<div className={styles.onboardingReviewPrompt}><div><strong>القناة الرسمية لرسائل المشتريات</strong><small>ألصق الرسالة مباشرة في الدردشة، أو استخدم نموذج الرسالة البنكية لاختيار الحساب يدويًا عند الحاجة.</small></div><button type="button" className={styles.secondaryButton} onClick={()=>openBankMessageDialog()}><LucideIcon name="receiptText" size={16}/><span>إضافة رسالة بنكية</span></button></div>}{activeRoomId==='secretary'&&<div className={styles.onboardingReviewPrompt}><div><strong>الحوكمة والاجتماعات في مكان واحد</strong><small>اعرض السياسات والصلاحيات والمحاضر أو اطلب اجتماعًا لمناقشة أي فقرة.</small></div><button type="button" className={styles.secondaryButton} onClick={()=>setGovernanceOpen(true)}><LucideIcon name="landmark" size={16}/><span>فتح مركز الحوكمة</span></button></div>}
    ...
    <GovernanceCenter open={governanceOpen} onClose={()=>setGovernanceOpen(false)} onboardingCompletedAt={onboardingCompletedAt} onOpenSecretary={()=>{setGovernanceOpen(false);chooseRoom('secretary')}}/>
