  ...
  return <div key={message.id} className={styles.messageClusterItem}>
    ...
    <footer className={styles.messageMeta}>
      ...
      {message.sender_type==='user'&&<span className={`${styles.messageReceipt} ${hasLaterAgentResponse?styles.messageReceiptRead:''}`} title={hasLaterAgentResponse?'مقروءة ومعالجة بواسطة نماء':'تم الإرسال'} aria-label={hasLaterAgentResponse?'مقروءة ومعالجة بواسطة نماء':'تم الإرسال'}>{hasLaterAgentResponse?'✓✓':'✓'}</span>}
        ...
        <form className={styles.composer} onSubmit={send}><input ref={statementFileRef} className={styles.hiddenFileInput} type="file" accept=".csv,text/csv" onChange={event=>{const file=event.target.files?.[0];if(file)void uploadStatement(file)}}/><button type="submit" className={styles.sendButton} disabled={!draft.trim()||sending} aria-label="إرسال"><span>{sending?'جارٍ التحليل…':'إرسال'}</span><LucideIcon name="send" size={20}/></button><textarea ref={composerTextareaRef} value={draft} onFocus={e=>{e.currentTarget.style.height='96px'}} onBlur={e=>{e.currentTarget.style.height='40px'}} onChange={e=>{const value=e.target.value;setDraft(value);e.currentTarget.style.height='auto';e.currentTarget.style.height=value.trim()?`${Math.min(Math.max(e.currentTarget.scrollHeight,40),96)}px`:'40px'}} placeholder={`اكتب إلى ${chatRoleTitle(activeRoom)}…`} rows={1} aria-label="نص الرسالة" maxLength={8000}/><button type="button" className={styles.attachButton} aria-label="إرفاق كشف حساب CSV" title="إرفاق كشف حساب CSV للمراجعة" onClick={()=>void prepareStatementUpload()} disabled={statementUploading}><LucideIcon name="upload" size={20}/></button></form>
      </main>
