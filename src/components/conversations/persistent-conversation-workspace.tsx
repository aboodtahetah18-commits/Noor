...
import { ALGORITHM_ROLE_REGISTRY, algorithmRolesForRoom, type AlgorithmRoleRef } from '@/lib/governance/algorithm-role-registry';
...
type AccountReconciliation={foundation_count:number;created_now:number;active_count:number;complete:boolean};
...
const RESPONSIBILITY_OWNERS=ALGORITHM_ROLE_REGISTRY.filter(role=>role.kind==='responsibility_owner');
...
export function PersistentConversationWorkspace(){
  ...
  async function loadManagedAccounts(){
    ...
    const data=await response.json() as {accounts?:ManagedAccount[];reconciliation?:AccountReconciliation};
    if(!response.ok)throw new Error('accounts');
  ...
  return <div key={message.id} className={styles.messageClusterItem}>
    ...
    {roomsOpen&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="القائمة الجانبية"><button type="button" className={styles.scrim} aria-label="إغلاق القائمة الجانبية" onClick={()=>setRoomsOpen(false)}/><aside className={styles.mobileSideSheet}><div className={styles.sideBrandRow}><span className={styles.sideBrandLogoWrap} aria-label="نماء"><Image className={`${styles.sideBrandLogo} ${styles.sideBrandLogoLight}`} src="/brand/ndos/namaa-logo-color-transparent.png" alt="" width={96} height={38}/><Image className={`${styles.sideBrandLogo} ${styles.sideBrandLogoDark}`} src="/brand/ndos/namaa-logo-white-transparent.png" alt="" width={96} height={38}/></span><ThemeToggle className={styles.sideThemeToggle}/><button type="button" className={styles.sideUserButton} aria-label="ملف المستخدم" aria-expanded={userMenuOpen} onClick={()=>setUserMenuOpen(open=>!open)}>{profile?.image?<span className={styles.userImage} style={{backgroundImage:`url("${profile.image.replace(/"/g,'')}")`}} aria-hidden="true"/>:<LucideIcon name="circleUserRound" size={20}/>}</button>{userMenuOpen&&<div className={styles.drawerUserMenu} role="dialog" aria-label="ملف المستخدم"><div className={styles.userMenuIdentity}><button type="button" className={styles.userMenuAvatar} aria-label="صورة المستخدم">{profile?.image?<span className={styles.userImage} style={{backgroundImage:`url("${profile.image.replace(/"/g,'')}")`}}/>:<LucideIcon name="circleUserRound" size={32}/>}</button><div><strong>{profile?.name||'المستخدم'}</strong><small>{profile?.email||''}</small></div></div><button type="button" onClick={()=>{setRoomsOpen(false);setUserMenuOpen(false);setProfileOpen(true)}}><LucideIcon name="pencil" size={20}/><span>الملف الشخصي وتعديل البيانات</span></button><button type="button" onClick={()=>{setRoomsOpen(false);setUserMenuOpen(false);setSettingsSection('general');setSettingsOpen(true)}}><LucideIcon name="settings" size={20}/><span>الإعدادات</span></button><button type="button" className={styles.logoutButton} onClick={()=>void logout()}><LucideIcon name="logOut" size={20}/><span>تسجيل الخروج</span></button></div>}</div>
      <div className={styles.sideSection}><small>مركز العمل</small>{onboardingComplete!==false?directoryTabs:null}{directoryContent}</div>
...
{detailTab==='records'&&<div className={styles.entityDetailPanel}><section><small>السجلات والتقارير المرجعية</small><EntityReferenceList items={recordRefs} roomId={room.id} onOpen={value=>{setActiveGovernedDocument(value);setDetailRoomId(null)}} icon="receiptText"/></section></div>}
    ...
    {settingsOpen&&<div className={`${styles.mobileOverlay} ${styles.accountSurfaceOverlay}`} role="dialog" aria-modal="true" aria-label="الإعدادات"><button type="button" className={`${styles.scrim} ${styles.accountSurfaceScrim}`} aria-label="إغلاق" onClick={()=>setSettingsOpen(false)}/><aside className={`${styles.mobileSheet} ${styles.mobileFullPageSheet} ${styles.accountSurfaceSheet}`}><div className={styles.sheetHeader}><strong>{settingsSection==='accounts'?'الحسابات':'الإعدادات'}</strong><button type="button" onClick={()=>setSettingsOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div>{settingsSection==='accounts'?<div className={styles.accountSettings}>
      <div className={styles.accountSettingsToolbar}>
        <div><strong>الحسابات المسجلة</strong><small>{managedAccounts.length} حسابات نشطة</small></div>
        <button type="button" className={styles.primaryActionButton} onClick={()=>setAccountEditor({mode:'add',name:'',bank_name:'',opening_balance:'0',effective_date:new Date().toISOString().slice(0,10)})}><LucideIcon name="plus" size={16}/><span>إضافة حساب</span></button>
      </div>
      {accountReconciliation&&<div className={styles.accountSyncStatus} role="status">
        <LucideIcon name={accountReconciliation.complete?'circleCheck':'info'} size={20}/>
        <span><strong>{accountReconciliation.complete?'تمت مطابقة حسابات التأسيس':'يوجد فرق بين حسابات التأسيس والحسابات النشطة'}</strong><small>المؤكد في التأسيس: {accountReconciliation.foundation_count} · الظاهر الآن: {accountReconciliation.active_count}{accountReconciliation.created_now>0?' · أضيف الآن: '+accountReconciliation.created_now:''}</small></span>
      </div>}
      {managedAccounts.length?<div className={styles.accountSettingsList}>{managedAccounts.map(account=><article key={account.id} className={styles.accountManageCard}>
        <div className={styles.accountManageIcon}><LucideIcon name="creditCard" size={20}/></div>
        <div className={styles.accountManageCopy}><div><strong>{account.name}</strong><small>{account.bank_name||account.account_type}</small></div><div className={styles.accountBalanceBlock}><small>الرصيد الحالي</small><strong>{formatSar(Number(account.balance||0))} ر.س</strong></div></div>
        <div className={styles.accountManageActions}><button type="button" onClick={()=>setAccountEditor({mode:'edit',id:account.id,name:account.name,bank_name:account.bank_name??'',opening_balance:account.opening_balance,effective_date:account.effective_date})} aria-label={'تعديل '+account.name}><LucideIcon name="pencil" size={16}/><span>تعديل</span></button><button type="button" onClick={()=>void deleteManagedAccount(account.id)} aria-label={'حذف '+account.name}><LucideIcon name="trash2" size={16}/><span>حذف</span></button></div>
      </article>)}</div>:<p>لا توجد حسابات مسجلة بعد.</p>}
