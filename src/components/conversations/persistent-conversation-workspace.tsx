...
export function PersistentConversationWorkspace(){
  ...
  return <div key={message.id} className={styles.messageClusterItem}>
    ...
    {detailRoomId&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="تفاصيل الجهة"><button type="button" className={styles.scrim} aria-label="إغلاق" onClick={()=>setDetailRoomId(null)}/><aside className={[styles.mobileSheet,styles.entityDetailSheet,'ux-dialog-surface'].join(' ')}><div className={styles.sheetHeader}><strong>ملف الجهة</strong><button type="button" onClick={()=>setDetailRoomId(null)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div>{(()=>{
