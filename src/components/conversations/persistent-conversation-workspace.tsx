...
export function PersistentConversationWorkspace(){
  ...
  async function openConversationContext(){
    ...
    try{
      ...
      // Context can still render from conversation state and attachments.
    }
  }
  ...
  async function send(event:FormEvent){ event.preventDefault(); const body=draft.trim(); if(!body||sending)return; setSending(true); setError('');
    try{
      ...
      setDraft('');
      if(composerTextareaRef.current) composerTextareaRef.current.style.height='38px';
    }
    catch{setError('لم تُحفظ الرسالة أو تعذر توليد الرد. لم يعتبر نماء الإرسال مكتملًا؛ أعد المحاولة.')} finally{setSending(false)} }

  const structuredOnboardingSteps=['dependents','income','accounts','obligations','goals'];
  const structuredOnboardingActive=onboardingComplete===false&&structuredOnboardingSteps.includes(onboardingStep??'');
  const structuredOnboardingLabel:Record<string,string>={
  ...
  return <section className={styles.page} dir="rtl" aria-label="محادثات نماء">
    ...
    <div className={`${styles.workspace} ${desktopRoomsVisible?'':styles.withoutRooms} ${desktopContextVisible?'':styles.withoutContext}`}>
      ...
      <main className={styles.chatPane}><header className={styles.chatHeader}><div className={styles.chatIdentity}><RoomPortrait room={activeRoom} size="md"/><div><div className={styles.entityTitle}><strong>{chatRoleTitle(activeRoom)}</strong></div><small>{chatEntityTitle(activeRoom)}</small></div></div><div className={styles.mobileTools}>{activeRoom.id==='central'&&<button type="button" className={styles.governorIntroButton} aria-label="التعرّف على المحافظ" onClick={()=>setDetailRoomId('central')}><LucideIcon name="circleUserRound" size={16}/><span>التعرّف على المحافظ</span></button>}<button type="button" aria-label="سياق المحادثة" onClick={()=>void openConversationContext()}><LucideIcon name="info" size={20}/></button></div></header>
        ...
        {structuredOnboardingActive&&<div className={styles.onboardingIntakeLauncher}><div><strong>{structuredOnboardingLabel[onboardingStep??'']??'بيانات التأسيس'}</strong><small>افتح النموذج، أكمل البيانات، ثم أكد المجموعة مرة واحدة.</small></div><button type="button" className={styles.primaryActionButton} onClick={()=>setIntakeOpen(true)}><LucideIcon name="listChecks" size={16}/><span>تعبئة البيانات</span></button></div>}
        ...
        <form className={styles.composer} onSubmit={send}><input ref={statementFileRef} className={styles.hiddenFileInput} type="file" accept=".csv,text/csv" onChange={event=>{const file=event.target.files?.[0];if(file)void uploadStatement(file)}}/><button type="button" className={styles.attachButton} aria-label="إرفاق كشف حساب CSV" title="إرفاق كشف حساب CSV للمراجعة" onClick={()=>void prepareStatementUpload()} disabled={statementUploading}><LucideIcon name="upload" size={20}/></button><textarea ref={composerTextareaRef} value={draft} onChange={e=>{setDraft(e.target.value);e.currentTarget.style.height='38px';e.currentTarget.style.height=`${Math.min(e.currentTarget.scrollHeight,96)}px`;}} placeholder={`اكتب إلى ${chatRoleTitle(activeRoom)}…`} rows={1} aria-label="نص الرسالة" maxLength={8000}/><button type="submit" className={styles.sendButton} disabled={!draft.trim()||sending} aria-label="إرسال"><span>{sending?'جارٍ التحليل…':'إرسال'}</span><LucideIcon name="send" size={20}/></button></form>
      </main>
      {desktopContextVisible&&<aside className={styles.contextPane} aria-label="سياق المحادثة"><div className={styles.paneTitle}><span>السياق</span><small>حيّز العمل</small></div><ConversationContextPanel
        roomKey={activeRoomId}
    ...
    {contextOpen&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="سياق المحادثة"><button type="button" className={styles.scrim} aria-label="إغلاق" onClick={()=>setContextOpen(false)}/><aside className={`${styles.mobileSheet} ${styles.contextSheet}`}><div className={styles.sheetHeader}><strong>سياق المحادثة</strong><button type="button" onClick={()=>setContextOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div><ConversationContextPanel
      roomKey={activeRoomId}
      participants={participants}
      attachments={attachments}
      facts={reviewFacts}
