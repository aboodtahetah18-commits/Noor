...
export function PersistentConversationWorkspace(){
  ...
  return <section className={`${styles.page} ${styles[`chatFont_${chatFontSize}`]}`} dir="rtl" aria-label="محادثات نماء">
    ...
    <div className={`${styles.workspace} ${styles.withoutRooms} ${desktopContextVisible?'':styles.withoutContext}`}>
      <main className={styles.chatPane}><header className={`${styles.chatHeader} ${activeRoom.building?styles.chatHeaderWithBuilding:''} ${activeRoom.id==='central'?styles.centralChatHeader:''}`}>{activeRoom.building&&<span className={styles.chatHeaderBuilding} aria-hidden="true"><Image src={activeRoom.building} alt="" fill sizes="100vw" priority={activeRoom.id==='central'}/></span>}<div className={styles.chatHeaderForeground}><div className={styles.chatIdentity}><RoomPortrait room={activeRoom} size={activeRoom.id==='central'?'lg':'md'}/><div><div className={styles.entityTitle}><strong>{chatRoleTitle(activeRoom)}</strong></div><small>{chatEntityTitle(activeRoom)}</small></div></div>{activeRoom.building&&<span className={styles.chatBuildingThumb} aria-hidden="true"><Image src={activeRoom.building} alt="" fill sizes="72px"/></span>}<div className={styles.mobileTools}><button type="button" aria-label="لوحة الجهة" onClick={()=>setEntityDashboardRoom(activeRoomId)}><LucideIcon name="chart" size={20}/></button><button type="button" aria-label="معلومات الجهة" onClick={()=>{setDetailRoomId(activeRoomId);setDetailTab('role')}}><LucideIcon name="info" size={20}/></button></div></div></header>
