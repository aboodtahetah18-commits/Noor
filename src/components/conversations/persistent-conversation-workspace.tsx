...
const rooms: [Room, ...Room[]] = [
  { id:'central', title:'بنك نماء المركزي', subtitle:'الحوكمة والاستقرار', lead:'محافظ نماء', specialists:'المحافظ وصاحب المسؤولية المختص، أو المستشار الاقتصادي عند الحاجة', avatar:'/brand/governor.webp', bankLogo:'/brand/bank-central.webp', building:'/brand/ndos/banks/namaa-central-bank-chat.webp' },
...
export function PersistentConversationWorkspace(){
  ...
  return <div key={message.id} className={styles.messageClusterItem}>
    ...
    {message.message_kind!=='message'&&message.structured_data?.onboarding!==true&&<section className={`${styles.structuredCard} ${styles[`kind_${message.message_kind}`]}`}>
      {!groupedWithPrevious&&<RichStructuredMessageHero room={activeRoom} kind={message.message_kind} data={message.structured_data} senderName={activeRoom.id==='central'?'محافظ نماء':message.sender_name}/>} 
