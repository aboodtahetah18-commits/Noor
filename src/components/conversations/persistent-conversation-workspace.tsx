...
function formatSar(value:number){return new Intl.NumberFormat('en-US',{maximumFractionDigits:2}).format(value)}
...
function formatMessageTimestamp(value?:string){if(!value)return '';const date=new Date(value);if(Number.isNaN(date.getTime()))return '';const day=String(date.getDate()).padStart(2,'0');const month=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][date.getMonth()];const year=date.getFullYear();const time=new Intl.DateTimeFormat('en-US',{hour:'numeric',minute:'2-digit',hour12:true}).format(date);return `${day} ${month} ${year} · ${time}`}
...
function OnboardingMessageContent({message,onOpenIntake,showIntakeButton=false}:{message:Message;onOpenIntake?:()=>void;showIntakeButton?:boolean}){
  ...
  return <div className={styles.onboardingMessageContent}>
    ...
    {question&&<div className={styles.onboardingQuestionBox}>
      ...
      {showIntakeButton&&onOpenIntake&&<button type="button" className={styles.inlineIntakeButton} onClick={onOpenIntake}><LucideIcon name="listChecks" size={16}/><span>فتح نموذج البيانات</span></button>}
    </div>}
    {!question&&<p>{body}</p>}
...
export function PersistentConversationWorkspace(){
  ...
  const [contextSection,setContextSection]=useState<'summary'|'data'|'files'|'permissions'>('summary');
  ...
  const activeDetail=governedRoomDetails[activeRoom.id];
  const contextTabs=<nav className={styles.contextTabs} aria-label="أقسام سياق المحادثة">
    <button type="button" className={contextSection==='summary'?styles.contextTabActive:''} onClick={()=>setContextSection('summary')}>الملخص</button>
    <button type="button" className={contextSection==='data'?styles.contextTabActive:''} onClick={()=>setContextSection('data')}>البيانات</button>
    <button type="button" className={contextSection==='files'?styles.contextTabActive:''} onClick={()=>setContextSection('files')}>الملفات</button>
    <button type="button" className={contextSection==='permissions'?styles.contextTabActive:''} onClick={()=>setContextSection('permissions')}>الصلاحيات</button>
  </nav>;
  const contextContent=<div className={styles.contextPage}>
    {contextSection==='summary'&&<>
      <section className={styles.contextCard}><small>الجهة الحالية</small><strong>{activeRoom.title}</strong><p>{activeRoom.lead} · {activeRoom.subtitle}</p></section>
      <section className={styles.contextCard}><small>المشاركون الفعليون</small><strong>{participants.length?`${participants.length} اختصاصيين`:'اختصاصيون حسب الموضوع'}</strong><p>{participants.length?participants.map(p=>p.display_name).join('، '):activeRoom.specialists}. لا تُستدعى جميع الجهات تلقائيًا.</p></section>
    ...
    {contextSection==='data'&&<>
      <section className={styles.contextCard}><small>حالة ملف التأسيس</small><strong>{onboardingComplete===true?'مكتمل':onboardingComplete===false?'قيد الاستكمال':'جارٍ التحقق'}</strong><p>{onboardingStep&&onboardingStep!=='complete'?`الخطوة الحالية: ${onboardingStep}`:'البيانات الأساسية متاحة للجهات المخولة فقط.'}</p></section>
      <button type="button" className={styles.contextAction} onClick={()=>{setContextOpen(false);setExtendedProfileOpen(true)}}><LucideIcon name="listChecks" size={16}/><span>فتح الملف المالي التفصيلي</span></button>
    </>}
    {contextSection==='files'&&<section className={styles.contextCard}><small>الملفات المشتركة</small><strong>{attachments.length?`${attachments.length} ملف`:'لا توجد ملفات'}</strong>{attachments.length?<div className={styles.contextFiles}>{attachments.map(file=><span key={file.id}><LucideIcon name="receiptText" size={16}/><b>{file.file_name}</b><em>{file.verification_status||'قيد المراجعة'}</em></span>)}</div>:<p>أي ملف ترفعه للمحادثة سيظهر هنا مع حالة مراجعته.</p>}</section>}
    {contextSection==='permissions'&&<>
      <section className={styles.contextCard}><small>مسؤولية الجهة</small><strong>{activeDetail.roleTitle}</strong><p>{activeDetail.responsibility}</p></section>
      <section className={styles.contextCard}><small>متى تتدخل؟</small><strong>وفق نطاقها فقط</strong><p>{activeDetail.intervention}</p></section>
      <section className={styles.contextCard}><small>حدود الصلاحية</small><strong>لا تنفيذ مالي تلقائي</strong><p>{activeDetail.governanceNote}</p></section>
    </>}
  </div>;
  ...
  return <section className={styles.page} dir="rtl" aria-label="محادثات نماء">
    ...
    <div className={`${styles.workspace} ${styles.withoutRooms} ${desktopContextVisible?'':styles.withoutContext}`}>
      <main className={styles.chatPane}><header className={styles.chatHeader}><div className={styles.chatIdentity}><RoomPortrait room={activeRoom} size="md"/><div><div className={styles.entityTitle}><strong>{chatRoleTitle(activeRoom)}</strong></div><small>{chatEntityTitle(activeRoom)}</small></div></div><div className={styles.mobileTools}><button type="button" aria-label="معلومات الجهة" onClick={()=>setContextOpen(true)}><LucideIcon name="info" size={20}/></button></div></header>
        ...
        <div className={styles.messages} aria-live="polite">{loading&&<p>جارٍ تحميل سجل المحادثة…</p>}{!loading&&!messages.length&&<article className={`${styles.message} ${styles.agentMessage}`}><p>{onboardingComplete===false?'أنا محافظ بنك نماء المركزي. سأبدأ معك بسؤال واحد في كل مرة حتى أبني ملفك من معلوماتك أنت، دون افتراضات.':'هذه بداية محادثتك مع '+activeRoom.title+'. اكتب سؤالك أو القرار الذي تريد دراسته.'}</p></article>}{messages.map(message=><article key={message.id} className={`${styles.message} ${message.sender_type==='user'?styles.userMessage:styles.agentMessage}`}><div className={`${styles.messageIdentity} ${message.sender_type==='user'?styles.userMessageIdentity:''}`}>{message.sender_type==='user'?<span className={styles.userMessageAvatar}>{profile?.image?<span className={styles.userImage} style={{backgroundImage:`url("${profile.image.replace(/"/g,'')}")`}} aria-hidden="true"/>:<LucideIcon name="circleUserRound" size={20}/>}</span>:<RoomPortrait room={activeRoom} size="sm"/>}<span><strong>{message.sender_type==='user'?(profile?.name||message.sender_name||'أنت'):(activeRoom.id==='central'?'محافظ البنك المركزي':message.sender_name)}</strong></span></div><div className={styles.messageBody}>{message.sender_type!=='user'&&message.structured_data?.onboarding===true?<OnboardingMessageContent message={message} showIntakeButton={['dependents','income','accounts','obligations','goals'].includes(onboardingStep??'')&&message.structured_data?.onboarding_step===onboardingStep} onOpenIntake={()=>setIntakeOpen(true)}/>:<p>{message.body}</p>}{message.message_kind!=='message'&&message.sender_type!=='user'&&message.structured_data?.onboarding!==true&&<section className={`${styles.structuredCard} ${styles[`kind_${message.message_kind}`]}`}><header><strong>{labels[message.message_kind]}</strong></header><StructuredFacts data={message.structured_data}/>{(message.message_kind==='decision'||message.message_kind==='request')&&<small className={styles.executionBoundary}>أي تنفيذ مالي خارجي يظل بيد المستخدم، ويحتاج تأكيدًا وإثباتًا قبل الإغلاق.</small>}</section>}</div>{message.created_at&&<footer className={styles.messageFooter}><time className={styles.messageTime} dateTime={message.created_at}>{formatMessageTimestamp(message.created_at)}</time></footer>}</article>)}</div>
        ...
        <form className={styles.composer} onSubmit={send}><input ref={statementFileRef} className={styles.hiddenFileInput} type="file" accept=".csv,text/csv" onChange={event=>{const file=event.target.files?.[0];if(file)void uploadStatement(file)}}/><button type="button" className={styles.attachButton} aria-label="إرفاق كشف حساب CSV" title="إرفاق كشف حساب CSV للمراجعة" onClick={()=>void prepareStatementUpload()} disabled={statementUploading}><LucideIcon name="upload" size={20}/></button><textarea ref={composerTextareaRef} value={draft} onChange={e=>setDraft(e.target.value)} placeholder={`اكتب إلى ${chatRoleTitle(activeRoom)}…`} rows={1} aria-label="نص الرسالة" maxLength={8000}/><button type="submit" className={styles.sendButton} disabled={!draft.trim()||sending} aria-label="إرسال"><span>{sending?'جارٍ التحليل…':'إرسال'}</span><LucideIcon name="send" size={20}/></button></form>
      </main>
      {desktopContextVisible&&<aside className={styles.contextPane} aria-label="سياق المحادثة"><div className={styles.paneTitle}><span>السياق</span><small>حيّز العمل</small></div>{contextTabs}{contextContent}</aside>}
    </div>
    {intakeOpen&&<div className={styles.intakeOverlay} role="dialog" aria-modal="true" aria-label="نموذج بيانات التأسيس"><button type="button" className={styles.scrim} aria-label="إغلاق نموذج البيانات" onClick={()=>setIntakeOpen(false)}/><aside className={styles.intakeDialog}><div className={styles.sheetHeader}><strong>نموذج البيانات</strong><button type="button" onClick={()=>setIntakeOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div><GovernorOnboardingIntake step={onboardingStep??''} onAccepted={(message,reply,nextStep)=>{setMessages(current=>[...current,message,...(reply?[reply]:[])]);setOnboardingStep(nextStep);setIntakeOpen(false);}}/></aside></div>}
    ...
    {contextOpen&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="سياق المحادثة"><button type="button" className={styles.scrim} aria-label="إغلاق" onClick={()=>setContextOpen(false)}/><aside className={`${styles.mobileSheet} ${styles.contextSheet}`}><div className={styles.sheetHeader}><strong>سياق المحادثة</strong><button type="button" onClick={()=>setContextOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div>{contextTabs}{contextContent}</aside></div>}
    ...
    {settingsOpen&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="الإعدادات"><button type="button" className={styles.scrim} aria-label="إغلاق" onClick={()=>setSettingsOpen(false)}/><aside className={`${styles.mobileSheet} ${styles.settingsSheet}`}><div className={styles.sheetHeader}><strong>{settingsSection==='accounts'?'الحسابات':'الإعدادات'}</strong><button type="button" onClick={()=>setSettingsOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div>{settingsSection==='accounts'?<div className={styles.accountSettings}>{statementAccounts.length?statementAccounts.map(account=><div key={account.id}><LucideIcon name="creditCard" size={20}/><span><strong>{account.name}</strong><small>{account.bank_name||account.account_type}</small></span></div>):<p>لا توجد حسابات مسجلة بعد. أضفها أثناء التأسيس مع المحافظ.</p>}</div>:<div className={styles.settingsList}><div className={styles.settingsCard}><span><strong>المظهر</strong><small>التبديل بين الوضع الفاتح والداكن.</small></span><ThemeToggle/></div><button type="button" className={styles.settingsCard} onClick={()=>{setSettingsOpen(false);setProfileOpen(true)}}><LucideIcon name="circleUserRound" size={20}/><span><strong>الملف الشخصي</strong><small>الاسم وبيانات الحساب الأساسية.</small></span><LucideIcon name="chevronLeft" size={16}/></button><button type="button" className={styles.settingsCard} onClick={()=>void openAccountsSettings()}><LucideIcon name="walletCards" size={20}/><span><strong>الحسابات</strong><small>الحسابات المسجلة والمستخدمة في نماء.</small></span><LucideIcon name="chevronLeft" size={16}/></button><div className={styles.settingsCard}><LucideIcon name="slidersHorizontal" size={20}/><span><strong>القيم الشخصية</strong><small>القيم القابلة للتخصيص فقط؛ الأوزان والحدود الحاكمة لا تعدل من الواجهة.</small></span></div></div>}</aside></div>}
