...
type Room = { id:RoomKey; title:string; subtitle:string; lead:string; specialists:string; avatar:string; bankLogo:string; building?:string|null };
const rooms: [Room, ...Room[]] = [
  { id:'central', title:'بنك نماء المركزي', subtitle:'الحوكمة والاستقرار', lead:'محافظ بنك نماء المركزي', specialists:'المحافظ وصاحب المسؤولية المختص، أو المستشار الاقتصادي عند الحاجة', avatar:'/brand/governor.webp', bankLogo:'/brand/bank-central.webp', building:'/brand/ndos/banks/namaa-central-bank.jpg' },
  { id:'operations', title:'مركز العمليات والمطابقة', subtitle:'الرسائل البنكية وكشوف الحسابات والمطابقة', lead:'مركز العمليات والمطابقة', specialists:'المطابقة والتسوية والتحقق مع استدعاء صاحب المسؤولية أو الجهة المختصة عند الحاجة', avatar:'/brand/governor.webp', bankLogo:'/brand/bank-central.webp', building:'/brand/ndos/banks/namaa-central-bank.jpg' },
  { id:'solvency', title:'بنك ملاءة', subtitle:'الحماية والاحتياطي', lead:'مدير بنك ملاءة', specialists:'مدير بنك ملاءة، ويستدعي مسؤول السيولة والحماية أو مسؤول الالتزامات عند صلة الموضوع', avatar:'/brand/malaa-manager.webp', bankLogo:'/brand/bank-malaa.webp', building:'/brand/ndos/banks/malaa-bank.jpg' },
  { id:'assets', title:'بنك الأصول الاستثماري', subtitle:'الأصول والأهداف والاستثمار', lead:'مدير بنك الأصول الاستثماري', specialists:'مدير بنك الأصول الاستثماري ومسؤول الاستثمار عند صلة الموضوع', avatar:'/brand/assets-manager.webp', bankLogo:'/brand/bank-assets.webp', building:'/brand/ndos/banks/investment-assets-bank.jpg' },
  { id:'hilal', title:'بنك الهلال', subtitle:'التمويل الداخلي', lead:'مدير بنك الهلال', specialists:'مدير بنك الهلال، ويستدعي مسؤول الالتزامات أو مسؤول الميزانية والإنفاق عند الحاجة', avatar:'/brand/hilal-manager.webp', bankLogo:'/brand/bank-hilal.webp', building:'/brand/ndos/banks/hilal-bank.jpg' },
  { id:'advisor', title:'المستشار الاقتصادي', subtitle:'تحليل الصورة المالية الكلية', lead:'المستشار الاقتصادي', specialists:'المستشار الاقتصادي، أو أحد أصحاب المسؤوليات المالية الخمسة بحسب موضوع الرسالة', avatar:'/brand/economic-advisor.webp', bankLogo:'/brand/namaa-logo.webp', building:'/brand/ndos/banks/namaa-central-bank.jpg' },
  { id:'secretary', title:'أمين السر المركزي', subtitle:'المحاضر والسياسات والاجتماعات', lead:'أمين السر المركزي', specialists:'أمين السر المركزي مع الجهة المختصة عند الحاجة', avatar:'/brand/governor.webp', bankLogo:'/brand/bank-central.webp', building:'/brand/ndos/banks/namaa-central-bank.jpg' },
  { id:'council', title:'مجلس نماء الأعلى', subtitle:'القرارات واللجان', lead:'محافظ بنك نماء المركزي بصفته رئيس المجلس', specialists:'أعضاء اللجنة ذات الصلة فقط، وليس جميع الشخصيات', avatar:'/brand/governor.webp', bankLogo:'/brand/bank-central.webp', building:'/brand/ndos/banks/namaa-central-bank.jpg' },
];
...
function compactChatRoleTitle(room:Room){
  ...
  if(room.id==='hilal')return 'مدير الهلال';
  if(room.id==='advisor')return 'المستشار الاقتصادي';
  if(room.id==='secretary')return 'أمين السر';
  if(room.id==='council')return 'رئيس المجلس';
  return room.lead;
}
...
function EntityReferenceList({items,roomId,onOpen,icon}:{items:GovernedDocumentRef[];roomId:RoomKey;onOpen:(value:{roomId:RoomKey;document:GovernedDocumentRef})=>void;icon:LucideIconName}){
  ...
  return <div className={styles.entityReferenceList}>
    {items.map(item=><button type="button" key={item.referenceCode} onClick={()=>onOpen({roomId,document:item})}>
      <span className={styles.entityReferenceIcon}><LucideIcon name={icon} size={16}/></span>
...
export function PersistentConversationWorkspace(){
  ...
  return <section className={`${styles.page} ${styles[`chatFont_${chatFontSize}`]}`} dir="rtl" aria-label="محادثات نماء">
    ...
    <div className={`${styles.workspace} ${styles.withoutRooms} ${desktopContextVisible?'':styles.withoutContext}`}>
      <main className={styles.chatPane}><header className={`${styles.chatHeader} ${activeRoom.building?styles.chatHeaderWithBuilding:''} ${activeRoom.id==='central'?styles.centralChatHeader:''}`}>{activeRoom.building&&<span className={styles.chatHeaderBuilding} aria-hidden="true"><Image src={activeRoom.building} alt="" fill sizes="100vw" priority={activeRoom.id==='central'}/></span>}<div className={styles.chatHeaderShade} aria-hidden="true"/><div className={styles.desktopChatHeaderForeground}><div className={styles.chatIdentity}><RoomPortrait room={activeRoom} size={activeRoom.id==='central'?'lg':'md'}/><div><div className={styles.entityTitle}><strong>{chatRoleTitle(activeRoom)}</strong></div><small>{chatEntityTitle(activeRoom)}</small></div></div>{activeRoom.building&&<span className={styles.chatBuildingThumb} aria-hidden="true"><Image src={activeRoom.building} alt="" fill sizes="72px"/></span>}</div><div className={styles.chatHeaderForeground}><button type="button" className={styles.compactMenuButton} aria-label="فتح القائمة الجانبية" onClick={()=>setRoomsOpen(true)}><LucideIcon name="menu" size={20}/></button><RoomPortrait room={activeRoom} size="md"/><div className={styles.compactRoleTitle}><strong>{compactChatRoleTitle(activeRoom)}</strong></div><div className={styles.mobileTools}><button type="button" aria-label="لوحة الجهة" onClick={()=>setEntityDashboardRoom(activeRoomId)}><LucideIcon name="chart" size={20}/></button><button type="button" aria-label="معلومات الجهة" onClick={()=>{setDetailRoomId(activeRoomId);setDetailTab('role')}}><LucideIcon name="info" size={20}/></button></div></div></header>
...
{detailTab==='team'&&<div className={styles.entityDetailPanel}><section><small>الفريق والأدوار الفعلية</small><div className={styles.entityPersonaBoard} aria-label="الهوية المعتمدة لشخصيات نماء"><Image src="/brand/ndos/personas/namaa-algorithmic-personas.jpg" alt="شخصيات نماء الخوارزمية المعتمدة" fill sizes="680px"/></div><div className={styles.entityTeamGrid}>{team.map(role=>{const portrait=rolePortraitByKey[role.key]??room.bankLogo;return <button type="button" key={role.referenceCode} onClick={()=>{setActiveAlgorithmRole({roomId:room.id,role});setDetailRoomId(null)}}><span className={styles.entityTeamPortrait}><Image src={portrait} alt="" fill sizes="52px"/></span><span><strong>{role.name}</strong><small>{role.kind==='responsibility_owner'?'صاحب مسؤولية مالية':role.kind==='advisor'?'مستشار اقتصادي':role.kind==='bank_manager'||role.kind==='central_bank_manager'?'إدارة البنك':'دور حوكمي'}</small></span><LucideIcon name="chevronLeft" size={16}/></button>})}</div></section><section><small>قاعدة الأدوار</small><p>كل شخصية تعمل داخل تفويضها المعتمد وهوية بنكها. المدير يقود نطاق البنك، وصاحب المسؤولية يحاسب على مجاله، والمستشار يقدم رأيًا دون سلطة تنفيذ أو اعتماد. التنفيذ المالي الخارجي يبقى بيد المستخدم.</p></section></div>}
...
{detailTab==='records'&&<div className={styles.entityDetailPanel}><section><small>السجلات والتقارير المرجعية</small><EntityReferenceList items={recordRefs} roomId={room.id} onOpen={value=>{setActiveGovernedDocument(value);setDetailRoomId(null)}} icon="receiptText"/></section></div>}
    ...
    {activeGovernedDocument&&<GovernedDocumentMobileSheet
      ...
      onClose={()=>{const roomId=activeGovernedDocument.roomId;const targetTab=classifyGovernedReference(activeGovernedDocument.document);setActiveGovernedDocument(null);setDetailRoomId(roomId);setDetailTab(targetTab)}}
    />}
