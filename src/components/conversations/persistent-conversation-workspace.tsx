...
const rooms: [Room, ...Room[]] = [
  { id:'central', title:'بنك نماء المركزي', subtitle:'الحوكمة والاستقرار', lead:'محافظ بنك نماء المركزي', specialists:'المحافظ وصاحب المسؤولية المختص، أو المستشار الاقتصادي عند الحاجة', avatar:'/brand/personas/central-governor.webp', bankLogo:'/brand/bank-central.webp', building:'/brand/ndos/banks/namaa-central-bank.jpg' },
  ...
  { id:'solvency', title:'بنك ملاءة', subtitle:'الحماية والاحتياطي', lead:'مدير بنك ملاءة', specialists:'مدير بنك ملاءة، ويستدعي مسؤول السيولة والحماية أو مسؤول الالتزامات عند صلة الموضوع', avatar:'/brand/personas/solvency-manager.webp', bankLogo:'/brand/bank-malaa.webp', building:'/brand/ndos/banks/malaa-bank.jpg' },
  { id:'assets', title:'بنك الأصول الاستثماري', subtitle:'الأصول والأهداف والاستثمار', lead:'مدير بنك الأصول الاستثماري', specialists:'مدير بنك الأصول الاستثماري ومسؤول الاستثمار عند صلة الموضوع', avatar:'/brand/personas/assets-manager.webp', bankLogo:'/brand/bank-assets.webp', building:'/brand/ndos/banks/investment-assets-bank.jpg' },
  { id:'hilal', title:'بنك الهلال', subtitle:'التمويل الداخلي', lead:'مدير بنك الهلال', specialists:'مدير بنك الهلال، ويستدعي مسؤول الالتزامات أو مسؤول الميزانية والإنفاق عند الحاجة', avatar:'/brand/personas/hilal-manager.webp', bankLogo:'/brand/bank-hilal.webp', building:'/brand/ndos/banks/hilal-bank.jpg' },
  { id:'advisor', title:'المستشار الاقتصادي', subtitle:'تحليل الصورة المالية الكلية', lead:'المستشار الاقتصادي', specialists:'المستشار الاقتصادي، أو أحد أصحاب المسؤوليات المالية الخمسة بحسب موضوع الرسالة', avatar:'/brand/personas/economic-advisor.webp', bankLogo:'/brand/ndos/namaa-logo-color-hq.png', building:'/brand/ndos/banks/namaa-central-bank.jpg' },
...
function RoomPortrait({room,size='md'}:{room:Room;size?:'sm'|'md'|'lg'}) {
  ...
  return <span className={`${styles.personaAvatar} ${styles[`persona_${size}`]} ${room.id==='central'?styles.centralPersona:''}`} aria-hidden="true">
    <Image src={portrait} alt="" fill unoptimized sizes={size==='lg'?'220px':size==='md'?'96px':'64px'} />
  </span>;
}
...
export function PersistentConversationWorkspace(){
  ...
  const [directoryMode,setDirectoryMode]=useState<'entities'|'owners'|'meetings'>('entities');
  ...
  const responsibilityRoles=onboardingComplete===false?[]:rooms.flatMap(room=>algorithmRolesForRoom(room.id)).filter((role,index,items)=>role.kind==='responsibility_owner'&&items.findIndex(item=>item.key===role.key)===index);
  ...
  const ownerButtons=<div className={styles.responsibilityList}>{responsibilityRoles.map(role=>{const portrait=rolePortraitByKey[role.key];return <button type="button" key={role.key} onClick={()=>{setRoomsOpen(false);setActiveAlgorithmRole({roomId:role.homeRoom as RoomKey,role})}}><span className={styles.responsibilityPortrait}>{portrait?<Image src={portrait} alt="" fill unoptimized sizes="64px"/>:<LucideIcon name="circleUserRound" size={22}/>}</span><span><strong>{role.name}</strong><small>صاحب مسؤولية مالية</small></span><LucideIcon name="info" size={18}/></button>})}</div>;
  const meetingDirectory=<div className={styles.meetingDirectoryList}>
    <button type="button" onClick={()=>{setRoomsOpen(false);setGovernanceMode('meetings')}}><LucideIcon name="calendarDays" size={20}/><span><strong>الاجتماعات واللجان</strong><small>المواعيد والمحاور والوثائق والبيانات المطلوبة</small></span><LucideIcon name="chevronLeft" size={16}/></button>
    <button type="button" onClick={()=>chooseRoom('council')}><RoomPortrait room={rooms.find(room=>room.id==='council')??rooms[0]} size="sm"/><span><strong>مجلس نماء الأعلى</strong><small>القرارات والاعتمادات العليا</small></span><LucideIcon name="chevronLeft" size={16}/></button>
    <button type="button" onClick={()=>chooseRoom('secretary')}><RoomPortrait room={rooms.find(room=>room.id==='secretary')??rooms[0]} size="sm"/><span><strong>أمين السر المركزي</strong><small>المحاضر وجدول الأعمال والمتابعات</small></span><LucideIcon name="chevronLeft" size={16}/></button>
  </div>;
  ...
  return <section className={`${styles.page} ${styles[`chatFont_${chatFontSize}`]}`} dir="rtl" aria-label="محادثات نماء">
    ...
    <div className={`${styles.workspace} ${desktopContextVisible?'':styles.withoutContext}`}>
      <aside className={styles.roomsPane} aria-label="الجهات والمحادثات"><div className={styles.paneTitle}><span>الجهات والمحادثات</span><small>مركز العمل</small></div>{roomButtons}</aside>
  ...
  return <div key={message.id} className={styles.messageClusterItem}>
    ...
    {roomsOpen&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="القائمة الجانبية"><button type="button" className={styles.scrim} aria-label="إغلاق القائمة الجانبية" onClick={()=>setRoomsOpen(false)}/><aside className={styles.mobileSideSheet}><div className={styles.sideBrandRow}><span className={styles.sideBrandLogoWrap} aria-label="نماء"><Image className={`${styles.sideBrandLogo} ${styles.sideBrandLogoLight}`} src="/brand/ndos/namaa-logo-color-transparent.png" alt="" width={96} height={38}/><Image className={`${styles.sideBrandLogo} ${styles.sideBrandLogoDark}`} src="/brand/ndos/namaa-logo-white-transparent.png" alt="" width={96} height={38}/></span><ThemeToggle className={styles.sideThemeToggle}/><button type="button" className={styles.sideUserButton} aria-label="ملف المستخدم" aria-expanded={userMenuOpen} onClick={()=>setUserMenuOpen(open=>!open)}>{profile?.image?<span className={styles.userImage} style={{backgroundImage:`url("${profile.image.replace(/"/g,'')}")`}} aria-hidden="true"/>:<LucideIcon name="circleUserRound" size={20}/>}</button>{userMenuOpen&&<div className={styles.drawerUserMenu} role="dialog" aria-label="ملف المستخدم"><div className={styles.userMenuIdentity}><button type="button" className={styles.userMenuAvatar} aria-label="صورة المستخدم">{profile?.image?<span className={styles.userImage} style={{backgroundImage:`url("${profile.image.replace(/"/g,'')}")`}}/>:<LucideIcon name="circleUserRound" size={32}/>}</button><div><strong>{profile?.name||'المستخدم'}</strong><small>{profile?.email||''}</small></div></div><button type="button" onClick={()=>{setRoomsOpen(false);setUserMenuOpen(false);setProfileOpen(true)}}><LucideIcon name="pencil" size={20}/><span>الملف الشخصي وتعديل البيانات</span></button><button type="button" onClick={()=>{setRoomsOpen(false);setUserMenuOpen(false);setSettingsSection('general');setSettingsOpen(true)}}><LucideIcon name="settings" size={20}/><span>الإعدادات</span></button><button type="button" className={styles.logoutButton} onClick={()=>void logout()}><LucideIcon name="logOut" size={20}/><span>تسجيل الخروج</span></button></div>}</div>
      <div className={styles.sideSection}><small>جهات الاتصال</small>
        <div className={styles.directoryTabs} role="tablist" aria-label="تصنيف جهات الاتصال">
          <button type="button" role="tab" aria-selected={directoryMode==='entities'} className={directoryMode==='entities'?styles.directoryTabActive:''} onClick={()=>setDirectoryMode('entities')}><LucideIcon name="landmark" size={16}/><span>الجهات</span></button>
          <button type="button" role="tab" aria-selected={directoryMode==='owners'} className={directoryMode==='owners'?styles.directoryTabActive:''} onClick={()=>setDirectoryMode('owners')}><LucideIcon name="circleUserRound" size={16}/><span>المسؤولون</span></button>
          <button type="button" role="tab" aria-selected={directoryMode==='meetings'} className={directoryMode==='meetings'?styles.directoryTabActive:''} onClick={()=>setDirectoryMode('meetings')}><LucideIcon name="calendarDays" size={16}/><span>الاجتماعات</span></button>
        </div>
        {directoryMode==='entities'?roomButtons:directoryMode==='owners'?ownerButtons:meetingDirectory}
      </div>
