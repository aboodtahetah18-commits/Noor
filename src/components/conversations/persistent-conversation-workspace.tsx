...
import { GovernanceHubPanel } from '@/components/conversations/governance-hub-panel';
import { governedRoomDetails } from '@/lib/conversations/governed-room-details';
import styles from './conversation-workspace.module.css';

type RoomKey = 'central' | 'solvency' | 'assets' | 'hilal' | 'advisor' | 'council' | 'operations' | 'governance' | 'secretary';
...
const rooms: [Room, ...Room[]] = [
  ...
  { id:'operations', title:'العمليات والمطابقة', subtitle:'رسائل المشتريات والحركات', lead:'مركز العمليات والمطابقة', specialists:'استخراج العملية وربط الحساب والبطاقة والتاجر ومنع التكرار', avatar:'/brand/namaa-logo.webp', bankLogo:'/brand/bank-central.webp' },
  { id:'governance', title:'الحوكمة والسياسات', subtitle:'السياسات والصلاحيات والآليات والسجلات', lead:'مركز الحوكمة والسياسات والسجلات', specialists:'النصوص الحاكمة والمحاضر والإصدارات وطلبات المراجعة', avatar:'/brand/namaa-logo.webp', bankLogo:'/brand/bank-central.webp' },
  { id:'secretary', title:'أمين السر والاجتماعات', subtitle:'المجلس واللجان والمحاضر', lead:'أمين السر المركزي', specialists:'جدولة الاجتماعات وتجهيز الحزم والمحاضر ومتابعة الإجراءات', avatar:'/brand/namaa-logo.webp', bankLogo:'/brand/bank-central.webp' },
];
...
export function PersistentConversationWorkspace(){
  ...
  const primaryRoomIds=new Set<RoomKey>(['central','solvency','assets','hilal','advisor','council']);
  const serviceRoomIds=new Set<RoomKey>(['operations','governance','secretary']);
  const renderRoom=(room:Room)=><div key={room.id} className={`${styles.roomItemShell} ${activeRoom.id===room.id?styles.activeRoom:''}`}><button type="button" onClick={()=>chooseRoom(room.id)} className={styles.roomItem}><RoomPortrait room={room} size="md"/><span className={styles.roomCopy}><strong>{chatRoleTitle(room)}</strong><small>{room.subtitle}</small></span></button><button type="button" className={styles.roomDetailButton} aria-label={`تفاصيل ${chatRoleTitle(room)}`} onClick={()=>setDetailRoomId(room.id)}><LucideIcon name="info" size={20}/></button></div>;
  const roomButtons=<div className={styles.roomList}>
    <div className={styles.roomGroup}><small>البنوك والمستشارون</small>{visibleRooms.filter(room=>primaryRoomIds.has(room.id)).map(renderRoom)}</div>
    {visibleRooms.some(room=>serviceRoomIds.has(room.id))&&<div className={styles.roomGroup}><small>الخدمات المركزية</small>{visibleRooms.filter(room=>serviceRoomIds.has(room.id)).map(renderRoom)}</div>}
  </div>;
  ...
  return <section className={styles.page} dir="rtl" aria-label="محادثات نماء">
    ...
    <div className={`${styles.workspace} ${desktopRoomsVisible?'':styles.withoutRooms} ${desktopContextVisible?'':styles.withoutContext}`}>
      ...
      <main className={`${styles.chatPane} ${mobileRoomList?styles.mobileChatHidden:''}`}><header className={styles.chatHeader}><div className={styles.chatIdentity}>{onboardingComplete!==false&&<button type="button" className={styles.mobileBack} aria-label="العودة إلى المحادثات" onClick={()=>setMobileRoomList(true)}><LucideIcon name="chevronRight" size={20}/></button>}<RoomPortrait room={activeRoom} size="md"/><div><div className={styles.entityTitle}><strong>{chatRoleTitle(activeRoom)}</strong></div><small>{chatEntityTitle(activeRoom)}</small></div></div><div className={styles.mobileTools}><button type="button" aria-label="معلومات الجهة" onClick={()=>setContextOpen(true)}><LucideIcon name="info" size={20}/></button></div></header>
        ...
        <div className={styles.attachmentPolicy}><LucideIcon name="upload" size={16}/><span>{activeRoomId==='operations'?'ألصق رسالة الشراء كما وصلت إليك. تحفظ مرة واحدة للمراجعة والمطابقة، ولا تعتبر إذنًا بتحريك المال.':onboardingStep==='statements'?'ارفع كشف CSV إن كان متاحًا. كل صف يبقى تحت المراجعة حتى تؤكده.':'المرفق للمراجعة والتحقق فقط؛ لا ينشئ حركة مالية ولا يثبت التنفيذ تلقائيًا.'}</span></div><div className={styles.executionNote}><LucideIcon name="circleCheck" size={16}/><span>نماء يوصي ويتابع؛ التنفيذ المالي الخارجي يتم بواسطة المستخدم.</span></div>
        {!['dependents','income','accounts','obligations','goals'].includes(onboardingStep??'')&&<form className={styles.composer} onSubmit={send}><input ref={statementFileRef} className={styles.hiddenFileInput} type="file" accept=".csv,text/csv" onChange={event=>{const file=event.target.files?.[0];if(file)void uploadStatement(file)}}/><button type="button" className={styles.attachButton} aria-label="إرفاق كشف حساب CSV" title="إرفاق كشف حساب CSV للمراجعة" onClick={()=>void prepareStatementUpload()} disabled={statementUploading}><LucideIcon name="upload" size={20}/></button><textarea ref={composerTextareaRef} value={draft} onChange={e=>{setDraft(e.target.value);e.currentTarget.style.height='40px';e.currentTarget.style.height=`${Math.min(e.currentTarget.scrollHeight,112)}px`;}} placeholder={activeRoomId==='operations'?'ألصق رسالة المشتريات هنا…':`اكتب إلى ${chatRoleTitle(activeRoom)}…`} rows={1} aria-label="نص الرسالة" maxLength={8000}/><button type="submit" className={styles.sendButton} disabled={!draft.trim()||sending} aria-label="إرسال"><span>{sending?'جارٍ التحليل…':'إرسال'}</span><LucideIcon name="send" size={20}/></button></form>}
      </main>
