...
type OnboardingReviewFact = { key:string; label:string; raw:string; editable?:boolean; verified_at?:string; confidence:number };
...
export function PersistentConversationWorkspace(){
  ...
  async function openConversationContext(){
    setContextOpen(true);
    try{
      ...
      if(response.ok) setReviewFacts(Array.isArray(data.facts)?data.facts:[]);
    }catch{
      // Context remains available even if foundation facts are temporarily unavailable.
    }
  }
  ...
  const roomButton=(room:Room)=><div key={room.id} className={`${styles.roomItemShell} ${activeRoom.id===room.id?styles.activeRoom:''}`}><button type="button" onClick={()=>chooseRoom(room.id)} className={styles.roomItem}><RoomPortrait room={room} size="md"/><span className={styles.roomCopy}><strong>{chatRoleTitle(room)}</strong></span></button><button type="button" className={styles.roomDetailButton} aria-label={`تفاصيل ${chatRoleTitle(room)}`} onClick={()=>setDetailRoomId(room.id)}><LucideIcon name="info" size={20}/></button></div>;
  const roomButtons=<div className={styles.roomList}>{visibleRooms.map(roomButton)}</div>;
  const groupedRooms=[
    {key:'banks',title:'البنوك',rooms:visibleRooms.filter(room=>['central','solvency','assets','hilal'].includes(room.id))},
    {key:'advisors',title:'المستشارون',rooms:visibleRooms.filter(room=>room.id==='advisor')},
    {key:'governance',title:'المجلس والاجتماعات',rooms:visibleRooms.filter(room=>room.id==='council')},
  ].filter(group=>group.rooms.length);
  const groupedRoomButtons=<div className={styles.sideGroups}>{groupedRooms.map(group=><section key={group.key} className={styles.sideGroup}><header><span>{group.title}</span><small>{group.rooms.length}</small></header><div className={styles.roomList}>{group.rooms.map(roomButton)}</div></section>)}</div>;

  const activeDetail=governedRoomDetails[activeRoomId];
  const importantMessages=messages.filter(message=>['risk','decision','recommendation','followup'].includes(message.message_kind)).slice(-3).reverse();
  const contextCards=<div className={styles.contextStack}>
    <section className={styles.contextCard}><small>الجهة الحالية</small><strong>{chatRoleTitle(activeRoom)}</strong><p>{chatEntityTitle(activeRoom)}</p></section>
    <section className={styles.contextCard}><small>المهام والاختصاص</small><strong>{activeDetail.responsibility}</strong><p>{activeDetail.intervention}</p></section>
    {activeRoomId==='central'&&<section className={styles.contextCard}><small>صلاحيات المحافظ</small><strong>يراقب · يحلل · يوصي · يعتمد مؤسسيًا حسب الحالة</strong><p>يقود التأسيس، يقيّم اكتمال الصورة، يحدد السؤال التالي، ويقدم التوصية العليا. لا يعتمد الحركة النقدية بدل المستخدم ولا يحرك أمواله.</p></section>}
    <section className={styles.contextCard}><small>البيانات المسجلة</small>{reviewFacts.length?<div className={styles.contextFacts}>{reviewFacts.slice(0,8).map(fact=><span key={fact.key}><b>{fact.label}</b><em>{fact.raw||'—'}</em></span>)}</div>:<p>لا توجد بيانات تأسيسية متاحة للعرض هنا حاليًا.</p>}</section>
    <section className={styles.contextCard}><small>الملفات</small>{attachments.length?<div className={styles.contextFiles}>{attachments.slice(0,6).map(file=><span key={file.id}><LucideIcon name="receiptText" size={16}/><b>{file.file_name}</b><em>{file.verification_status||'قيد المراجعة'}</em></span>)}</div>:<p>لا توجد ملفات مشتركة في هذه المحادثة حتى الآن.</p>}</section>
    <section className={styles.contextCard}><small>ملاحظات أحتاج الرجوع لها</small>{importantMessages.length?<div className={styles.contextHighlights}>{importantMessages.map(message=><p key={message.id}>{message.body}</p>)}</div>:<p>لا توجد ملاحظات حاكمة أو متابعات مثبتة في هذه المحادثة حتى الآن.</p>}</section>
    <section className={styles.contextCard}><small>المشاركون الفعليون</small><strong>{participants.length?`${participants.length} مشاركين`:'حسب الموضوع'}</strong><p>{participants.length?participants.map(p=>p.display_name).join('، '):activeRoom.specialists}</p></section>
    <section className={styles.contextCard}><small>حد التنفيذ</small><strong>توصية ومتابعة فقط</strong><p>لا تحويل، لا سداد، ولا تنفيذ مالي خارجي يُعد منفذًا من المنصة.</p></section>
  </div>;
  ...
  return <section className={styles.page} dir="rtl" aria-label="محادثات نماء">
    ...
    {roomsOpen&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="القائمة الجانبية"><button type="button" className={styles.scrim} aria-label="إغلاق" onClick={()=>setRoomsOpen(false)}/><aside className={styles.mobileSideSheet}><div className={styles.sideBrandHeader}><div><Image src="/brand/ndos/namaa-logo-color-transparent.png" alt="نماء" width={92} height={44}/><small>الجهات والمحادثات</small></div><button type="button" onClick={()=>setRoomsOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div><div className={styles.sidePattern} aria-hidden="true"><Image src="/brand/namaa-leaf.webp" alt="" width={180} height={180}/></div><div className={styles.sideSection}>{groupedRoomButtons}</div><div className={styles.sideUtilityList}>{onboardingComplete!==false&&<button type="button" onClick={()=>void openAccountsSettings()}><LucideIcon name="walletCards" size={20}/><span>الحسابات</span></button>}<button type="button" onClick={()=>{setRoomsOpen(false);setSettingsSection('general');setSettingsOpen(true)}}><LucideIcon name="settings" size={20}/><span>الإعدادات</span></button><button type="button" onClick={()=>{setRoomsOpen(false);void openConversationContext()}}><LucideIcon name="info" size={20}/><span>المساعدة والسياق</span></button></div></aside></div>}
