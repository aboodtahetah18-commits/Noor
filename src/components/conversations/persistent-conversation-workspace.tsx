  ...
  return <div key={message.id} className={styles.messageClusterItem}>
    ...
    <footer className={styles.messageMeta}>
      ...
      {message.sender_type==='user'&&<span className={`${styles.messageReceipt} ${hasLaterAgentResponse?styles.messageReceiptRead:''}`} title={hasLaterAgentResponse?'مقروءة ومعالجة بواسطة نماء':'تم الإرسال'} aria-label={hasLaterAgentResponse?'مقروءة ومعالجة بواسطة نماء':'تم الإرسال'}>{hasLaterAgentResponse?'✓✓':'✓'}</span>}
        ...
        <form className={styles.composer} onSubmit={send}><input ref={statementFileRef} className={styles.hiddenFileInput} type="file" accept=".csv,text/csv" onChange={event=>{const file=event.target.files?.[0];if(file)void uploadStatement(file)}}/><button type="submit" className={styles.sendButton} disabled={!draft.trim()||sending} aria-label="إرسال"><span>{sending?'جارٍ التحليل…':'إرسال'}</span><LucideIcon name="send" size={20}/></button><textarea ref={composerTextareaRef} value={draft} onFocus={e=>{e.currentTarget.style.height=draft.trim()?`${Math.max(e.currentTarget.scrollHeight,64)}px`:'64px'}} onBlur={e=>{if(!draft.trim())e.currentTarget.style.height='40px'}} onChange={e=>{const value=e.target.value;setDraft(value);e.currentTarget.style.height='auto';e.currentTarget.style.height=value.trim()?`${Math.max(e.currentTarget.scrollHeight,64)}px`:'64px'}} placeholder={`اكتب إلى ${chatRoleTitle(activeRoom)}…`} rows={1} aria-label="نص الرسالة" maxLength={8000}/><button type="button" className={styles.attachButton} aria-label="إرفاق كشف حساب CSV" title="إرفاق كشف حساب CSV للمراجعة" onClick={()=>void prepareStatementUpload()} disabled={statementUploading}><LucideIcon name="upload" size={20}/></button></form>
      </main>
    ...
    {detailRoomId&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="تفاصيل الجهة"><button type="button" className={styles.scrim} aria-label="إغلاق" onClick={()=>setDetailRoomId(null)}/><aside className={`${styles.mobileSheet} ${styles.entityDetailSheet}`}><div className={styles.sheetHeader}><strong>ملف الجهة</strong><button type="button" onClick={()=>setDetailRoomId(null)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div>{(()=>{const room=rooms.find(item=>item.id===detailRoomId)??rooms[0];const detail=governedRoomDetails[room.id];const roomFiles=room.id===activeRoomId?attachments:[];const openGovernance=()=>{setDetailRoomId(null);setGovernanceMode('governance')};return <div className={styles.roomDetailContent}><div className={styles.roomDetailHero}><RoomPortrait room={room} size="lg"/><div><strong>{detail.roleTitle}</strong><small>{detail.entityTitle}</small></div></div><nav className={styles.entityDetailTabs} aria-label="أقسام ملف الجهة"><button type="button" className={detailTab==='role'?styles.activeEntityDetailTab:''} onClick={()=>setDetailTab('role')}>الاختصاص</button><button type="button" className={detailTab==='files'?styles.activeEntityDetailTab:''} onClick={()=>setDetailTab('files')}>الملفات</button><button type="button" className={detailTab==='records'?styles.activeEntityDetailTab:''} onClick={()=>setDetailTab('records')}>السجلات والسياسات</button></nav>{detailTab==='role'&&<div className={styles.entityDetailPanel}><section><small>المسؤولية الأساسية</small><p>{detail.responsibility}</p></section><section><small>ما الذي يراقبه؟</small><p>{detail.observes}</p></section><section><small>متى يتدخل؟</small><p>{detail.intervention}</p></section><section><small>متى لا يتدخل؟</small><p>{detail.avoids}</p></section><section><small>حدود الحوكمة والصلاحيات</small><p>{detail.governanceNote}</p></section></div>}{detailTab==='files'&&<div className={styles.entityDetailPanel}><section><small>الملفات المرتبطة بهذه الجهة</small>{roomFiles.length?<div className={styles.detailFiles}>{roomFiles.map(file=><span key={file.id}><LucideIcon name="receiptText" size={16}/><b>{file.file_name}</b><em>{file.verification_status||'قيد المراجعة'}</em></span>)}</div>:<p>لا توجد ملفات مشتركة مسجلة في هذه المحادثة حاليًا.</p>}</section><section><small>المرجع الحاكم</small><div className={styles.detailList}>{detail.sourceRefs.map(ref=><button type="button" key={ref} onClick={()=>setActiveEntityReference({roomId:room.id,ref})}><LucideIcon name="receiptText" size={16}/><span>{ref}</span><LucideIcon name="chevronLeft" size={16}/></button>)}</div></section></div>}{detailTab==='records'&&<div className={styles.entityDetailPanel}><section><small>السجلات المهمة</small>{detail.records.length?<div className={styles.detailList}>{detail.records.map(item=><button type="button" key={item.title} onClick={()=>item.sourceUrl?window.open(item.sourceUrl,'_blank','noopener,noreferrer'):openGovernance()}><LucideIcon name="listChecks" size={16}/><span>{item.title}</span><LucideIcon name="chevronLeft" size={16}/></button>)}</div>:<p>لا توجد سجلات مرجعية مستقلة في الحزمة النافذة لهذه الجهة.</p>}</section><section><small>السياسات المتعلقة بهذه الجهة</small>{detail.policies.length?<div className={styles.detailList}>{detail.policies.map(item=><button type="button" key={item.title} onClick={()=>item.sourceUrl?window.open(item.sourceUrl,'_blank','noopener,noreferrer'):openGovernance()}><LucideIcon name="landmark" size={16}/><span>{item.title}</span><LucideIcon name="chevronLeft" size={16}/></button>)}</div>:<p>لا توجد سياسات مرجعية مستقلة في الحزمة النافذة لهذه الجهة.</p>}</section></div>}</div>})()}</aside></div>}
