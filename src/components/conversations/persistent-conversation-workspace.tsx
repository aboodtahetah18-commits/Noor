...
import { governedRoomDetails, type GovernedDocumentRef } from '@/lib/conversations/governed-room-details';
import { GovernedDocumentMobileSheet } from '@/components/conversations/governed-document-mobile-sheet';
import { AlgorithmRoleMobileSheet } from '@/components/conversations/algorithm-role-mobile-sheet';
import { EntityDashboardMobilePage } from '@/components/conversations/entity-dashboard-mobile-page';
...
function messageKindIcon(kind:MessageKind):LucideIconName{
  ...
  if(kind==='followup')return 'listChecks';
...
export function PersistentConversationWorkspace(){
  ...
  const [activeEntityReference,setActiveEntityReference]=useState<{roomId:RoomKey;ref:string}|null>(null);
  ...
  },[activeRoomId,loadedRoomId,messages.length]);
  ...
  return <div key={message.id} className={styles.messageClusterItem}>
    ...
    <footer className={styles.messageMeta}>
      ...
      {message.sender_type==='user'&&<span className={`${styles.messageReceipt} ${hasLaterAgentResponse?styles.messageReceiptRead:''}`} title={hasLaterAgentResponse?'مقروءة ومعالجة بواسطة نماء':'تم الإرسال'} aria-label={hasLaterAgentResponse?'مقروءة ومعالجة بواسطة نماء':'تم الإرسال'}>{hasLaterAgentResponse?'✓✓':'✓'}</span>}
        ...
        />
        {statementPickerOpen&&<div className={styles.statementPicker}><div><strong>اختر الحساب المرتبط بالكشف</strong><small>سيُقرأ الملف للمراجعة فقط، ولن ينشئ معاملات تلقائيًا.</small></div><select value={statementAccountId} onChange={event=>setStatementAccountId(event.target.value)} aria-label="الحساب المرتبط بكشف الحساب">{statementAccounts.map(account=><option key={account.id} value={account.id}>{account.bank_name||account.name} — {account.name}</option>)}</select><button type="button" className={styles.secondaryButton} onClick={()=>statementFileRef.current?.click()} disabled={statementUploading}>{statementUploading?'جارٍ الاستيراد…':'اختيار ملف كشف الحساب'}</button></div>}
        <div className={styles.attachmentPolicy}><LucideIcon name="upload" size={16}/><span>{onboardingStep==='statements'?'ارفع ملف كشف الحساب إن كان متاحًا. كل صف يبقى تحت المراجعة حتى تؤكده.':'المرفق للمراجعة والتحقق فقط؛ لا ينشئ حركة مالية ولا يثبت التنفيذ تلقائيًا.'}</span></div><div className={styles.executionNote}><LucideIcon name="circleCheck" size={16}/><span>نماء يوصي ويتابع؛ التنفيذ المالي الخارجي يتم بواسطة المستخدم.</span></div>
        <form className={styles.composer} onSubmit={send}><input ref={statementFileRef} className={styles.hiddenFileInput} type="file" accept=".csv,text/csv" onChange={event=>{const file=event.target.files?.[0];if(file)void uploadStatement(file)}}/><button type="submit" className={styles.sendButton} disabled={!draft.trim()||sending} aria-label="إرسال"><span>{sending?'جارٍ التحليل…':'إرسال'}</span><LucideIcon name="send" size={20}/></button><textarea ref={composerTextareaRef} value={draft} onFocus={e=>{e.currentTarget.style.height='96px'}} onBlur={e=>{e.currentTarget.style.height='40px'}} onChange={e=>{const value=e.target.value;setDraft(value);if(document.activeElement===e.currentTarget){e.currentTarget.style.height='auto';e.currentTarget.style.height=Math.min(Math.max(e.currentTarget.scrollHeight,96),144)+'px'}}} placeholder={`اكتب إلى ${chatRoleTitle(activeRoom)}…`} rows={1} aria-label="نص الرسالة" maxLength={8000}/><button type="button" className={styles.attachButton} aria-label="إرفاق كشف حساب" title="إرفاق كشف حساب للمراجعة" onClick={()=>void prepareStatementUpload()} disabled={statementUploading}><LucideIcon name="upload" size={20}/></button></form>
      </main>
    ...
    {activeGovernedDocument&&<GovernedDocumentMobileSheet
      ...
      onClose={()=>{const roomId=activeGovernedDocument.roomId;setActiveGovernedDocument(null);setDetailRoomId(roomId);setDetailTab('records')}}
    />}
    ...
    />}
    {activeEntityReference&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="المرجع الحاكم"><button type="button" className={styles.scrim} aria-label="إغلاق" onClick={()=>setActiveEntityReference(null)}/><aside className={`${styles.mobileSheet} ${styles.entityReferenceSheet}`}><div className={styles.sheetHeader}><strong>المرجع الحاكم</strong><button type="button" onClick={()=>setActiveEntityReference(null)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div>{(()=>{const room=rooms.find(item=>item.id===activeEntityReference.roomId)??rooms[0];const detail=governedRoomDetails[room.id];return <div className={styles.entityReferenceContent}><span>{activeEntityReference.ref}</span><strong>{detail.roleTitle}</strong><small>{detail.entityTitle}</small><p>{detail.governanceNote}</p><section><small>صلة المرجع بالجهة</small><p>{detail.responsibility}</p></section><button type="button" className={styles.primaryActionButton} onClick={()=>{setActiveEntityReference(null);setDetailRoomId(null);setGovernanceMode('governance')}}><LucideIcon name="landmark" size={20}/><span>فتح مركز الحوكمة والسياسات</span></button></div>})()}</aside></div>}
    ...
    {profileOpen&&<div className={`${styles.mobileOverlay} ${styles.accountSurfaceOverlay}`} role="dialog" aria-modal="true" aria-label="الملف الشخصي"><button type="button" className={`${styles.scrim} ${styles.accountSurfaceScrim}`} aria-label="إغلاق" onClick={()=>setProfileOpen(false)}/><aside className={`${styles.mobileSheet} ${styles.accountSurfaceSheet}`}><div className={styles.sheetHeader}><strong>الملف الشخصي</strong><button type="button" onClick={()=>setProfileOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div><div className={styles.profileForm}><label><span>الاسم</span><input value={profileName} onChange={event=>setProfileName(event.target.value)} maxLength={120}/></label><label><span>البريد الإلكتروني</span><input value={profile?.email??''} readOnly/></label><small>تغيير البريد أو كلمة المرور يمر عبر مسار أمان الحساب ولا يُعدّل من شاشة الدردشة مباشرة.</small><button type="button" className={styles.primaryActionButton} onClick={()=>void saveProfile()} disabled={profileSaving||profileName.trim().length<2}>{profileSaving?'جارٍ الحفظ…':'حفظ التعديل'}</button></div></aside></div>}
    {settingsOpen&&<div className={`${styles.mobileOverlay} ${styles.accountSurfaceOverlay}`} role="dialog" aria-modal="true" aria-label="الإعدادات"><button type="button" className={`${styles.scrim} ${styles.accountSurfaceScrim}`} aria-label="إغلاق" onClick={()=>setSettingsOpen(false)}/><aside className={`${styles.mobileSheet} ${styles.accountSurfaceSheet}`}><div className={styles.sheetHeader}><strong>{settingsSection==='accounts'?'الحسابات':'الإعدادات'}</strong><button type="button" onClick={()=>setSettingsOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div>{settingsSection==='accounts'?<div className={styles.accountSettings}>{statementAccounts.length?statementAccounts.map(account=><div key={account.id}><LucideIcon name="creditCard" size={20}/><span><strong>{account.name}</strong><small>{account.bank_name||account.account_type}</small></span></div>):<p>لا توجد حسابات مسجلة بعد. أضفها أثناء التأسيس مع المحافظ.</p>}</div>:<div className={styles.settingsList}>
