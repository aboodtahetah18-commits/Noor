...
const rooms: [Room, ...Room[]] = [
  { id:'central', title:'بنك نماء المركزي', subtitle:'الحوكمة والاستقرار', lead:'محافظ بنك نماء المركزي', specialists:'المحافظ وأحد المستشارين الستة المعتمدين فقط عند الحاجة', avatar:'/brand/governor.webp', bankLogo:'/brand/bank-central.webp' },
  { id:'operations', title:'مركز العمليات والمطابقة', subtitle:'الرسائل البنكية وكشوف الحسابات والمطابقة', lead:'مسؤول مركز العمليات والمطابقة', specialists:'المطابقة والتصنيف والتسوية مع استدعاء الجهة المختصة عند الحاجة', avatar:'/brand/governor.webp', bankLogo:'/brand/bank-central.webp' },
  { id:'solvency', title:'بنك ملاءة', subtitle:'الحماية والاحتياطي', lead:'مدير بنك ملاءة', specialists:'مدير بنك ملاءة، ويستدعي مستشار السيولة أو مستشار الالتزامات عند صلة الموضوع', avatar:'/brand/malaa-manager.webp', bankLogo:'/brand/bank-malaa.webp' },
  ...
  { id:'hilal', title:'بنك الهلال', subtitle:'التمويل الداخلي', lead:'مدير بنك الهلال', specialists:'مدير بنك الهلال، ويستدعي مستشار الالتزامات أو مستشار الميزانية والإنفاق عند الحاجة', avatar:'/brand/hilal-manager.webp', bankLogo:'/brand/bank-hilal.webp' },
  { id:'advisor', title:'المستشار الاقتصادي', subtitle:'تحليل الصورة المالية الكلية', lead:'المستشار الاقتصادي', specialists:'المستشار الاقتصادي، أو أحد المستشارين الستة المعتمدين بحسب موضوع الرسالة', avatar:'/brand/economic-advisor.webp', bankLogo:'/brand/namaa-logo.webp' },
  ...
  return <section className={`${styles.page} ${styles[`chatFont_${chatFontSize}`]}`} dir="rtl" aria-label="محادثات نماء">
    ...
    <div className={`${styles.workspace} ${styles.withoutRooms} ${desktopContextVisible?'':styles.withoutContext}`}>
      <main className={styles.chatPane}><header className={styles.chatHeader}><div className={styles.chatIdentity}><RoomPortrait room={activeRoom} size="md"/><div><div className={styles.entityTitle}><strong>{chatRoleTitle(activeRoom)}</strong></div><small>{chatEntityTitle(activeRoom)}</small></div></div><div className={styles.mobileTools}><button type="button" aria-label="معلومات الجهة" onClick={()=>setContextOpen(true)}><LucideIcon name="info" size={20}/></button></div></header>
        ...
        />}
        {onboardingComplete===false&&intakeDismissed&&['dependents','income','accounts','obligations','goals'].includes(onboardingStep??'')&&<button type="button" className={styles.resumeIntakeButton} onClick={()=>setIntakeDismissed(false)}><LucideIcon name="listChecks" size={18}/><span>متابعة استكمال البيانات</span></button>}
    ...
    {settingsOpen&&<div className={styles.mobileOverlay} role="dialog" aria-modal="true" aria-label="الإعدادات"><button type="button" className={styles.scrim} aria-label="إغلاق" onClick={()=>setSettingsOpen(false)}/><aside className={styles.mobileSheet}><div className={styles.sheetHeader}><strong>{settingsSection==='accounts'?'الحسابات':'الإعدادات'}</strong><button type="button" onClick={()=>setSettingsOpen(false)} aria-label="إغلاق"><LucideIcon name="x" size={20}/></button></div>{settingsSection==='accounts'?<div className={styles.accountSettings}>{statementAccounts.length?statementAccounts.map(account=><div key={account.id}><LucideIcon name="creditCard" size={20}/><span><strong>{account.name}</strong><small>{account.bank_name||account.account_type}</small></span></div>):<p>لا توجد حسابات مسجلة بعد. أضفها أثناء التأسيس مع المحافظ.</p>}</div>:<div className={styles.settingsList}>
      ...
      <section className={styles.settingsSectionBlock}><div className={styles.settingsSectionHeading}><LucideIcon name="type" size={20}/><span><strong>حجم نص الدردشة</strong><small>غيّره فورًا دون التأثير على بقية الواجهة.</small></span></div><div className={styles.fontSizeChoices}>{(['small','medium','large'] as ChatFontSize[]).map(size=><button key={size} type="button" className={chatFontSize===size?styles.activeFontChoice:''} onClick={()=>updateChatFontSize(size)}>{size==='small'?'صغير':size==='medium'?'متوسط':'كبير'}</button>)}</div></section>
      <section className={styles.settingsSectionBlock}><div className={styles.settingsSectionHeading}><LucideIcon name="landmark" size={20}/><span><strong>الحوكمة والسجلات</strong><small>السياسات، مصفوفة الصلاحيات، القرارات، المحاضر والاجتماعات في مكان واحد.</small></span></div>{onboardingComplete!==false?<div className={styles.settingsActionGrid}><button type="button" onClick={()=>{setSettingsOpen(false);setGovernanceMode('governance')}}><LucideIcon name="shieldCheck" size={18}/><span>السياسات والصلاحيات والقرارات</span></button><button type="button" onClick={()=>{setSettingsOpen(false);setGovernanceMode('meetings')}}><LucideIcon name="calendarDays" size={18}/><span>الاجتماعات والمحاضر</span></button><button type="button" onClick={()=>{setSettingsOpen(false);setGovernanceMode('documents')}}><LucideIcon name="receiptText" size={18}/><span>الوثائق والتقارير</span></button></div>:<small className={styles.settingsLockedNote}>تفتح هذه الأقسام بعد اعتماد بيانات التأسيس مع المحافظ.</small>}</section>
      <section className={styles.settingsSectionBlock}><div className={styles.settingsSectionHeading}><LucideIcon name="slidersHorizontal" size={20}/><span><strong>ضبط النظام</strong><small>إعدادات العرض والتنبيهات والخصوصية قابلة للتخصيص؛ أما الأوزان والحدود المحكومة فتظل تحت الحوكمة ولا تعدّل من الواجهة.</small></span></div><div className={styles.settingsStatusList}><span><b>الجهات والبنوك والمستشارون</b><em>{onboardingComplete===false?'تفتح بعد اكتمال التأسيس':'مفتوحة'}</em></span><span><b>الاجتماعات</b><em>{onboardingComplete===false?'تظهر بعد اعتماد التأسيس':'متاحة'}</em></span><span><b>التنفيذ المالي</b><em>بيد المستخدم فقط</em></span></div></section>
    </div>}</aside></div>}
