...
export function PersistentConversationWorkspace(){
  ...
  return <section className={styles.page} dir="rtl" aria-label="محادثات نماء">
    <header className={styles.workspaceHeader}><div className={styles.headingCopy}><span className={styles.eyebrow}>محادثات نماء</span><h1>مركز الحوار والقرار</h1><p>المحادثات محفوظة في حسابك، وتصل رسالتك إلى الجهة والمتخصصين المرتبطين بالموضوع.</p></div><div className={styles.headerActions}><button type="button" className={styles.secondaryButton} onClick={()=>setDesktopRoomsVisible(v=>!v)}><LucideIcon name="layoutGrid" size={17}/><span>{desktopRoomsVisible?'إخفاء الجهات':'إظهار الجهات'}</span></button><button type="button" className={styles.secondaryButton} onClick={()=>setDesktopContextVisible(v=>!v)}><LucideIcon name="info" size={17}/><span>{desktopContextVisible?'إخفاء السياق':'إظهار السياق'}</span></button></div></header>
    <div className={`${styles.workspace} ${desktopRoomsVisible?'':styles.withoutRooms} ${desktopContextVisible?'':styles.withoutContext}`}>
      ...
      <main className={styles.chatPane}><header className={styles.chatHeader}><div className={styles.chatIdentity}><span className={styles.entityAvatar}>{activeRoom.title.slice(0,1)}</span><div><div className={styles.entityTitle}><strong>{activeRoom.title}</strong><span>شخصية خوارزمية</span></div><small>{activeRoom.subtitle}</small></div></div><div className={styles.mobileTools}><button type="button" aria-label="فتح الجهات" onClick={()=>setRoomsOpen(true)}><LucideIcon name="messageSquareText" size={20}/></button><button type="button" aria-label="فتح سياق المحادثة" onClick={()=>setContextOpen(true)}><LucideIcon name="info" size={20}/></button></div></header>
        <div className={styles.routingNote}><LucideIcon name="sparkles" size={15}/><span><strong>التوجيه الذكي:</strong> {activeRoom.specialists}.</span></div>
        ...
        {error&&<div className={styles.routingNote} role="alert"><LucideIcon name="triangleAlert" size={15}/><span>{error}</span></div>}
        <div className={styles.attachmentPolicy}><LucideIcon name="upload" size={15}/><span>المرفق للمراجعة والتحقق فقط؛ لا ينشئ حركة مالية ولا يثبت التنفيذ تلقائيًا.</span></div><div className={styles.executionNote}><LucideIcon name="circleCheck" size={15}/><span>نماء يوصي ويتابع؛ التنفيذ المالي الخارجي يتم بواسطة المستخدم.</span></div>
        <form className={styles.composer} onSubmit={send}><button type="button" className={styles.attachButton} aria-label="إرفاق ملف" title="الإرفاق سيُفعّل بعد ربط التخزين الآمن"><LucideIcon name="upload" size={19}/></button><textarea value={draft} onChange={e=>setDraft(e.target.value)} placeholder={`اكتب إلى ${activeRoom.title}…`} rows={1} aria-label="نص الرسالة" maxLength={8000}/><button type="submit" className={styles.sendButton} disabled={!draft.trim()||sending}><span>{sending?'جارٍ الحفظ…':'إرسال'}</span><LucideIcon name="chevronLeft" size={18}/></button></form>
      </main>
