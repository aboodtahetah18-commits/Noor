    ...
    <aside className={styles.sheet} dir="rtl">
      ...
      <div className={styles.cards}>
        {section.items.map((item,index)=><details key={`${section.id}-${item.title}-${index}`} className={styles.card} open={index===0}>
          <summary><span><strong>{item.title}</strong>{item.meta&&<small>{item.meta}</small>}</span><LucideIcon name="chevronDown" size={16}/></summary>
          <p>{item.body}</p>
      ...
      <footer className={styles.footer}>
        <button type="button" className={styles.primary} onClick={onOpenSecretary}><LucideIcon name="messageSquareText" size={16}/><span>اسأل أمين السر أو اطلب اجتماعًا</span></button>
