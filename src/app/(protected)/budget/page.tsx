...
export default async function BudgetPage(){
  ...
  return <main className="p47-page" dir="rtl"><section className="p47-content-shell">
    <PageHeader className="p47-page-heading namaa-financial-header namaa-migrated-header" eyebrow={`خطة الإنفاق الشهرية · ${cycle.name}`} title="الميزانية" meta={<div className="p47-cycle-line"><span className={`p47-status-dot ${center.overBudgetCount?'is-danger':'is-good'}`}/><span>{planStatus[plan.status]??plan.status}</span><b>•</b><span>{center.overBudgetCount?`${center.overBudgetCount} بند متجاوز`:'لا يوجد تجاوز مؤكد'}</span></div>} actions={<><Link href="/budget/optimizer" className="ux-button ux-button--secondary">تحسين توزيع الراتب</Link>{plan.status==='ACTIVE_PLAN'?<Link href={`/budget/revise?plan=${plan.id}`} className="ux-button ux-button--primary">تعديل الخطة</Link>:null}</>}/>
