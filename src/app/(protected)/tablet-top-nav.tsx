...
const items=[['/dashboard','الرئيسية'],['/transactions','الحركة المالية'],['/budget','التخطيط'],['/goals','الأهداف'],['/advisor','المستشار'],['/more','المزيد']] as const;
...
export function TabletTopNav(){
  const pathname=usePathname();
    ...
    <nav className="mustaqbali-tablet-nav" aria-label="التنقل الرئيسي للتابلت">{items.map(([href,label])=>{const active=href==='/dashboard'?pathname==='/dashboard':pathname.startsWith(href);return <Link key={href} href={href} className={active?'is-active':''} aria-current={active?'page':undefined}>{label}</Link>;})}</nav>
  </header>;
}
