...
export default async function NewTransferPage({searchParams}:{searchParams:Promise<{error?:string}>}){
  ...
  return <main className="app-page" dir="rtl"><section className="page-shell narrow-shell p47-flow-page p47-flow-card"><p className="eyebrow">تحويل داخلي</p><div className="title-with-help"><h1>تحويل بين الحسابات</h1></div>{q.error&&<p className="p47-inline-error" role="alert">{q.error}</p>}
    <form className="p47-flow-form p73-entry-form p73-transfer-form" action={transferAction} data-financial-calc="source-balance"><input type="hidden" name="cycleId" value={cycle.id}/><input type="hidden" name="idempotencyKey" value={randomUUID()}/>
      <label>من الحساب<select name="fromAccountId" required defaultValue=""><option value="" disabled>اختر الحساب المصدر</option>{accounts.map(a=><option key={a.id} value={a.id} data-balance={a.balance}>{a.name} — {a.balance} ريال</option>)}</select></label>
      ...
      <output className="financial-auto-result" data-financial-output>رصيد الحساب بعد العملية: 0.00 ريال</output><button className="primary-button" type="submit">تنفيذ التحويل</button>
    </form></section></main>;
}
