...
export default async function HistoryReportsPage({ searchParams }: { searchParams: Promise<{ window?: string }> }) {
  ...
  return <main className="app-page reports-page p47-analysis-page namaa-history-page" dir="rtl"><div className="page-shell dashboard-shell namaa-migrated-shell">
    <PageHeader className="p47-analysis-header namaa-migrated-header" eyebrow="التقارير" title="التحليل التاريخي" description="قارن الدورات المغلقة واتجاهات الدخل والمصروف والادخار والفائض من بيانات فعلية محفوظة." actions={<><Link className="ux-button ux-button--secondary" href="/reports">الدورة الحالية</Link><Link className={window===3?'ux-button ux-button--primary':'ux-button ux-button--secondary'} href="/reports/history?window=3">آخر 3 دورات</Link><Link className={window===6?'ux-button ux-button--primary':'ux-button ux-button--secondary'} href="/reports/history?window=6">آخر 6 دورات</Link></>}/>
    ...
    {history.items.length===0 ? <section className="empty-state"><h2>لا توجد دورات مغلقة بعد</h2><p>سيبدأ التحليل التاريخي بعد إغلاق أول دورة وحفظ نتائجها.</p></section> : <>
