...
const MESSAGE:Record<string,string>={
  ...
  GRANT_STATUS_REQUEST_CREATED:'تم إنشاء طلب تغيير حالة Grant.',
  ...
  return <main className="app-page p47-decision-page" dir="rtl">
    <div className="page-shell p47-analysis-shell">
      <header className="p47-analysis-header">
        <div><p className="eyebrow">Authorization Governance</p><h1>إدارة الصلاحيات والتفويضات</h1><p className="muted">كل تغيير يمر عبر طلب، اعتماد مستقل، ثم تطبيق ذري. DENY هو الوضع الافتراضي.</p></div>
      </header>
      ...
      <div className="p74-inline-metrics">
        ...
        <span>Grants فعالة <b>{snapshot.grants.filter((r)=>r.is_active===true).length}</b></span>
        <span>تفويضات فعالة <b>{countBy(snapshot.delegations,'status','ACTIVE')}</b></span>
        <span>Break Glass نشط <b>{activeBreakGlass.length}</b></span>
      </div>

      <section className="p47-analysis-card">
        <div className="p47-section-heading"><div><span>Provisioning Inbox</span><h2>صندوق طلبات الصلاحيات</h2></div><small>الطالب لا يستطيع اعتماد طلبه بنفسه</small></div>
        ...
        {[...pending,...approved].map((r)=><details className="decision-details" key={text(r,'id')} open={String(r.status)==='PENDING'}>
          <summary>{text(r,'change_type')} — {text(r,'status')}</summary>
          <div className="p74-inline-metrics">
            <span>الطالب <b>{text(r,'requested_by_name')}</b></span>
            <span>المستفيد <b>{text(r,'subject_name')}</b></span>
            <span>المعرف <b>{text(r,'id')}</b></span>
            <span>تاريخ الطلب <b>{text(r,'requested_at')}</b></span>
          </div>
          <p>{text(r,'rationale')}</p>
          {String(r.status)==='PENDING'?<div className="ndos-actions">
            ...
            <form action={rejectProvisioningRequestAction} className="ndos-form-grid"><input type="hidden" name="requestId" value={text(r,'id')}/><textarea name="rationale" minLength={20} required placeholder="سبب الرفض القابل للتدقيق"/><button className="secondary-button" type="submit">رفض الطلب</button></form>
          </div>:null}
          {String(r.status)==='APPROVED'?<form action={applyProvisioningRequestAction} className="ndos-actions"><input type="hidden" name="requestId" value={text(r,'id')}/><button className="primary-button" type="submit">تطبيق الطلب ذريًا</button></form>:null}
        </details>)}
      </section>

      <section className="p47-analysis-card">
        <div className="p47-section-heading"><div><span>Create Requests</span><h2>إنشاء طلبات حوكمية جديدة</h2></div><small>هذه النماذج لا تمنح صلاحية مباشرة</small></div>

        <details className="decision-details"><summary>طلب إسناد دور</summary>
          <form action={createRoleAssignmentRequestAction} className="ndos-form-grid">
            ...
            <div className="ndos-actions"><button className="primary-button" type="submit">إنشاء طلب الدور</button></div>
          </form>
        </details>

        <details className="decision-details"><summary>طلب Grant</summary>
          <form action={createGrantRequestAction} className="ndos-form-grid">
            <label><span>الدور</span><select name="role" required>{ROLES.map((v)=><option key={v}>{v}</option>)}</select></label>
            ...
            <label><span>أقصى مخاطرة</span><select name="maxRisk"><option value="">بدون حد إضافي</option>{LEVELS.map((v)=><option key={v}>{v}</option>)}</select></label>
            ...
            <label><span>المسوغ</span><textarea name="rationale" required minLength={20}/></label>
            <div className="ndos-actions"><button className="primary-button" type="submit">إنشاء طلب Grant</button></div>
          </form>
        </details>

        <details className="decision-details"><summary>طلب تفويض محدود</summary>
          <form action={createDelegationRequestAction} className="ndos-form-grid">
            ...
            <label><span>المسوغ</span><textarea name="rationale" required minLength={20}/></label>
            <div className="ndos-actions"><button className="primary-button" type="submit">إنشاء طلب التفويض</button></div>
          </form>
        </details>

        <details className="decision-details"><summary>طلب Break Glass</summary>
          <form action={createBreakGlassRequestAction} className="ndos-form-grid">
            <label><span>المستخدم</span><select name="userId" required>{directory.map((u)=><option key={u.id} value={u.id}>{u.displayName}</option>)}</select></label>
            ...
            <div className="ndos-actions"><button className="secondary-button" type="submit">إنشاء طلب Break Glass</button></div>
          </form>
        </details>
      </section>

      <section className="p47-analysis-card">
        <div className="p47-section-heading"><div><span>Emergency Access</span><h2>Break Glass النشط</h2></div><small>حد أقصى 60 دقيقة</small></div>
        {activeBreakGlass.length===0?<div className="p47-empty-state"><strong>لا يوجد وصول طارئ نشط</strong></div>:activeBreakGlass.map((r)=><details className="decision-details" key={text(r,'id')} open>
          <summary>{text(r,'actor_name')} — {text(r,'incident_reference')}</summary>
          <div className="p74-inline-metrics"><span>ينتهي خلال <b><BreakGlassCountdown expiresAt={text(r,'expires_at')}/></b></span><span>ينتهي في <b>{text(r,'expires_at')}</b></span><span>المعتمد <b>{text(r,'approved_by_name')}</b></span></div>
          <p>{text(r,'justification')}</p>
          <form action={revokeBreakGlassAction} className="ndos-form-grid"><input type="hidden" name="sessionId" value={text(r,'id')}/><textarea name="rationale" required minLength={20} placeholder="سبب الإلغاء الفوري"/><div className="ndos-actions"><button className="secondary-button" type="submit">إلغاء الوصول الطارئ</button></div></form>
        </details>)}
      </section>

      <section className="p47-analysis-card">
        <div className="p47-section-heading"><div><span>Active Access</span><h2>الأدوار والصلاحيات والتفويضات</h2></div></div>
        <details className="decision-details"><summary>Role Assignments ({snapshot.assignments.length})</summary>{snapshot.assignments.map((r)=><div key={text(r,'id')} className="p47-empty-state"><strong>{text(r,'display_name')} — {text(r,'role')}</strong><span>{text(r,'status')} | Bank: {text(r,'bank_key')} | Committee: {text(r,'committee_id')}</span>{['ACTIVE','SUSPENDED'].includes(text(r,'status'))?<form action={requestAssignmentStatusAction} className="ndos-form-grid"><input type="hidden" name="assignmentId" value={text(r,'id')}/><select name="status" defaultValue={text(r,'status')==='ACTIVE'?'SUSPENDED':'ACTIVE'}><option>ACTIVE</option><option>SUSPENDED</option><option>REVOKED</option><option>EXPIRED</option></select><input name="rationale" required minLength={20} placeholder="مسوغ تغيير الحالة"/><button className="secondary-button" type="submit">طلب تغيير الحالة</button></form>:null}</div>)}</details>
        <details className="decision-details"><summary>Grants ({snapshot.grants.length})</summary>{snapshot.grants.map((r)=><div key={text(r,'id')} className="p47-empty-state"><strong>{text(r,'role')} — {text(r,'action')} / {text(r,'object_type')}</strong><span>{r.is_active===true?'ACTIVE':'INACTIVE'} | Bank: {text(r,'bank_key')} | Max risk: {text(r,'max_risk')} | Max amount: {text(r,'max_amount')}</span><form action={requestGrantStatusAction} className="ndos-form-grid"><input type="hidden" name="grantId" value={text(r,'id')}/><input type="hidden" name="isActive" value={r.is_active===true?'false':'true'}/><input name="rationale" required minLength={20} placeholder="مسوغ تغيير حالة Grant"/><button className="secondary-button" type="submit">طلب {r.is_active===true?'تعطيل':'تفعيل'}</button></form></div>)}</details>
        <details className="decision-details"><summary>Delegations ({snapshot.delegations.length})</summary>{snapshot.delegations.map((r)=><div key={text(r,'id')} className="p47-empty-state"><strong>{text(r,'from_name')} → {text(r,'to_name')} — {text(r,'to_role')}</strong><span>{text(r,'status')} | {text(r,'starts_at')} → {text(r,'ends_at')}</span>{['ACTIVE','SUSPENDED'].includes(text(r,'status'))?<form action={requestDelegationStatusAction} className="ndos-form-grid"><input type="hidden" name="delegationId" value={text(r,'id')}/><select name="status" defaultValue={text(r,'status')==='ACTIVE'?'SUSPENDED':'ACTIVE'}><option>ACTIVE</option><option>SUSPENDED</option><option>REVOKED</option><option>EXPIRED</option></select><input name="rationale" required minLength={20} placeholder="مسوغ تغيير حالة التفويض"/><button className="secondary-button" type="submit">طلب تغيير الحالة</button></form>:null}</div>)}</details>
      </section>

      <section className="p47-analysis-card">
        <div className="p47-section-heading"><div><span>Immutable Audit</span><h2>سجل إدارة الصلاحيات</h2></div><small>Append-only</small></div>
        {snapshot.events.map((r)=><details className="decision-details" key={text(r,'id')}><summary>{text(r,'event_type')} — {text(r,'created_at')}</summary><div className="p74-inline-metrics"><span>الفاعل <b>{text(r,'actor_name')}</b></span><span>الهدف <b>{text(r,'target_name')}</b></span><span>الكائن <b>{text(r,'object_type')} / {text(r,'object_id')}</b></span></div><p>{text(r,'reason')}</p></details>)}
      </section>
    </div>
  </main>;
}
