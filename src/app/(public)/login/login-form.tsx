...
export function LoginForm({ returnTo }: { returnTo: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.form} aria-label="تسجيل الدخول">
      ...
      <div className={styles.metaRow}>
        <p className={styles.helper}><Link href="/forgot-password">نسيت كلمة المرور؟</Link></p>
      </div>

      {error ? <p className={styles.alert} role="alert">{error}</p> : null}

      <button className={styles.submit} type="submit" disabled={pending}>
        <span>{pending ? 'جاري الدخول...' : 'دخول إلى نماء'}</span>
        <LucideIcon name="chevronLeft" size={20}/>
      </button>

      <p className={styles.createAccount}>ليس لديك حساب؟ <Link href="/register">إنشاء حساب جديد</Link></p>
    </form>
  );
}
