...
export async function GET(){
  ...
  try{
    ...
    const meetings=[
      ...
      {id:'assets-cycle',title:'لجنة الاستثمار والأصول',kind:'لجنة دائمة',scheduled_at:addDays(anchor,7).toISOString(),cadence:'اليوم السابع من كل ثالث دورة',status:'دوري'},
      {id:'governance-cycle',title:'لجنة السياسات والمخاطر والتدقيق',kind:'لجنة دائمة',scheduled_at:addDays(anchor,10).toISOString(),cadence:'اليوم العاشر من كل ثالث دورة',status:'دوري'},
    ];
    return NextResponse.json({onboarding_complete:true,anchor:completedAt.toISOString(),meetings});
  }catch(error){
