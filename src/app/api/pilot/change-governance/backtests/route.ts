...
import { recordAlgorithmBacktest } from '@/features/pilot/services/algorithm-governance-write-service';
...
const schema=z.object({
  ...
  metricsText:z.string().trim().min(20).max(12000),
  evidenceText:z.string().trim().min(20).max(12000),
...
export async function POST(request:Request){
  ...
  try{
    ...
    enforceRateLimit(`pilot-governance-backtest:${user.id}`);
    const parsed=schema.safeParse(await request.json().catch(()=>null));
    if(!parsed.success)return NextResponse.json({ok:false,error:'INVALID_REQUEST'},{status:422,headers});
    const result=await recordAlgorithmBacktest({userId:user.id,...parsed.data});
    ...
    const requestId=logServerError('pilot-governance-backtest-failed',{endpoint:'/api/pilot/change-governance/backtests',userId:user.id});
